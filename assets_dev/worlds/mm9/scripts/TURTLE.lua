...
script.labels["HandleOnAlert"] = function(ctx)
    ...
    ctx:state().sAlertName = ctx:object("hAlertedBy"):className() -- TURTLE.scr:26
...
script.labels["TurtleOnDamage"] = function(ctx)
    ...
    -- Alert nearby AI
    ctx:self():sendAlert(ctx:object("hAttacker")) -- TURTLE.scr:37
...
script.labels["InitTurtle"] = function(ctx)
    ...
    if ctx:condition("g_nDistance != 0") then -- TURTLE.scr:48
        ctx:set("MAX_DIST_FROM_STARTPOINT", "g_nDistance") -- TURTLE.scr:49
    else -- TURTLE.scr:50
        ctx:state().MAX_DIST_FROM_STARTPOINT = 350 -- TURTLE.scr:51
    end -- TURTLE.scr:52
    -- Set how often we stop at an obstacle
    ctx:state().g_nStopAtObstacle = 2 -- TURTLE.scr:55
    -- Set how often we idle 1 equals 10% valid numbers are 1 to 10
    ctx:state().g_IdleFrequency = 4 -- TURTLE.scr:58
    -- Set max time in seconds before idle check
    ctx:state().g_IdleCheckMin = 7 -- TURTLE.scr:61
    ctx:state().g_IdleCheckMax = 7 -- TURTLE.scr:62
    -- Set how often special anim is played valid numbers are 1 to 10
    ctx:state().g_SpecialAnimFrequency = 0 -- TURTLE.scr:65
...
script.labels["Main"] = function(ctx)
    ...
    ctx:onEvent("OnDamage", "TurtleOnDamage") -- TURTLE.scr:74

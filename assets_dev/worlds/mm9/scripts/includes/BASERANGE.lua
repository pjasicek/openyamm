...
script.labels["BaseRangeGoGetHim"] = function(ctx)
    -- BASERANGE.inc:22
    ctx:getTime("g_nTemp") -- BASERANGE.inc:25
    ctx:state().lastAttackTime = 0 -- BASERANGE.inc:26
    ctx:state().g_bTemp = ctx:self():canReachTarget() -- BASERANGE.inc:28
    ...
    ctx:wait("ATTACK_CHECK_WAIT", "GO_AFTER_MIN_WAIT_TIME", "BaseRangeAttackCheck") -- BASERANGE.inc:35
...
script.labels["BaseRangeAttackDone"] = function(ctx)
    ...
    if ctx:condition("lastAttackTime!=0") then -- BASERANGE.inc:53
        ctx:getTime("g_nTemp") -- BASERANGE.inc:54
        ctx:sub("g_nTemp", "TARGET_HIT_WAIT_TIME") -- BASERANGE.inc:55
        if ctx:condition("g_nTemp > lastAttackTime") then -- BASERANGE.inc:56
            if ctx:condition("g_nTemp > lastTargetHit") then -- BASERANGE.inc:57
                ctx:state().lastAttackTime = 0 -- BASERANGE.inc:58
                ctx:state().lastTargetHit = 0 -- BASERANGE.inc:59
                -- Move after him....
...
script.labels["BaseDoRangeAttack"] = function(ctx)
    ...
    ctx:state().g_bFighting = true -- BASERANGE.inc:76
    if ctx:condition("lastAttackTime==0") then -- BASERANGE.inc:78
        ctx:getTime("lastAttackTime") -- BASERANGE.inc:79
    end -- BASERANGE.inc:80
    ctx:self():rangeAttack("BaseRangeAttackDone") -- BASERANGE.inc:82
...
script.labels["BaseRangeMaybeAttack"] = function(ctx)
    ...
    ctx:state().g_bCanAttack = ctx:self():canAttack() -- BASERANGE.inc:101
    ...
    ctx:state().g_nTemp = 0 -- BASERANGE.inc:107
    ctx:self():estimateRangeAttackHit(ctx:object("g_hObject")) -- BASERANGE.inc:108
    if ctx:condition("g_hObject==NULL") then -- BASERANGE.inc:109
        ctx:state().g_nTemp = (tonumber(ctx:state().g_nTemp) or 0) + 20 -- BASERANGE.inc:110
    else -- BASERANGE.inc:111
        if ctx:condition("g_hObject==g_hTarget") then -- BASERANGE.inc:112
            ctx:state().g_nTemp = 100 -- BASERANGE.inc:113
        else -- BASERANGE.inc:114
            ctx:state().g_bTemp = ctx:object("g_hObject"):isClass("AIBase") -- BASERANGE.inc:115
            if ctx:condition("g_bTemp==TRUE") then -- BASERANGE.inc:116
                ctx:state().g_nTemp = 0 -- BASERANGE.inc:117
    ...
    ctx:randomInt(0, 100, "g_nRandom") -- BASERANGE.inc:126
...
script.labels["BaseRangeFoundPlayer"] = function(ctx)
    ...
    ctx:self():setTarget(ctx:object("g_hTarget")) -- BASERANGE.inc:156
    ctx:randomInt(0, 100, "g_nRandom") -- BASERANGE.inc:158
    ctx:self():setTarget(ctx:object("g_hTarget")) -- BASERANGE.inc:160
    ctx:self():aware("BaseRangeAwareDone") -- BASERANGE.inc:161
...
script.labels["BaseRangeOnProjectile"] = function(ctx)
    ...
    if ctx:condition("g_hAttacker!=NULL") then -- BASERANGE.inc:182
        ctx:self():unlink(ctx:object("g_hAttacker")) -- BASERANGE.inc:183
    ...
    ctx:set("g_hTarget", "g_hAttacker") -- BASERANGE.inc:196
    ctx:self():setTarget(ctx:object("g_hTarget")) -- BASERANGE.inc:197
...
script.labels["BaseRangeAttackCheckDone"] = function(ctx)
    -- BASERANGE.inc:218
    ctx:randomFloat(1.3, 2.0, "g_nRandom") -- BASERANGE.inc:219
    ctx:wait("ATTACK_CHECK_WAIT", "g_nRandom", "BaseRangeAttackCheck") -- BASERANGE.inc:220
...
script.labels["ChaseTargetTick"] = function(ctx)
    ...
    ctx:state().g_nDist1 = ctx:self():aiDistanceTo(ctx:object("g_hTarget")) -- BASERANGE.inc:240
    ctx:state().g_nTemp = ctx:self():getStat("RangeAttackRange") -- BASERANGE.inc:242
    ...
    ctx:self():estimateRangeAttackHit(ctx:object("g_hObject")) -- BASERANGE.inc:249
    ctx:state().g_bTemp = 0 -- BASERANGE.inc:251
...
script.labels["ChaseTargetTickDone"] = function(ctx)
    -- BASERANGE.inc:261
    ctx:wait("CHASE_TARGET_WAIT", 1.0, "ChaseTargetTick") -- BASERANGE.inc:262
...
script.labels["ChangeTargetWait"] = function(ctx)
    ...
    ctx:state().g_posX, ctx:state().g_posY, ctx:state().g_posZ = ctx:self():pos() -- BASERANGE.inc:274
    ctx:state().g_nDist1 = ctx:self():aiDistanceTo(ctx:object("g_hTarget")) -- BASERANGE.inc:275
    ctx:self():findTargets("hTargetArray", 16, "g_nTemp", "g_nDist1", 0) -- BASERANGE.inc:277
    ...
    ctx:arrayGet("hTargetArray", 0, "g_hObject") -- BASERANGE.inc:283
    if ctx:condition("g_hObject==g_hTarget") then -- BASERANGE.inc:285
        ...
        ctx:arrayGet("hTargetArray", 1, "g_hObject") -- BASERANGE.inc:289
    end -- BASERANGE.inc:290
    ctx:set("g_hTarget", "g_hObject") -- BASERANGE.inc:292
    ctx:self():setTarget(ctx:object("g_hTarget")) -- BASERANGE.inc:293
end

script.labels["ChangeTargetWaitDone"] = function(ctx)
    -- BASERANGE.inc:295
    ctx:wait("CHANGE_TARGET_WAIT", 1.5, "ChangeTargetWait") -- BASERANGE.inc:297
...
script.labels["BaseRangeTargetHit"] = function(ctx)
    ...
    ctx:getTime("lastTargetHit") -- BASERANGE.inc:313
...
script.labels["BaseRangeInit"] = function(ctx)
    ...
    ctx:onEvent("OnFoundPlayer", "BaseRangeFoundPlayer") -- BASERANGE.inc:326
    ctx:onEvent("OnProjectile", "BaseRangeOnProjectile") -- BASERANGE.inc:327
    ctx:onEvent("OnTargetHit", "BaseRangeTargetHit") -- BASERANGE.inc:328
    ctx:wait("ATTACK_CHECK_WAIT", 1.5, "BaseRangeAttackCheck") -- BASERANGE.inc:330
    -- Note, we're replacing the base.inc chase target wait with
    -- our own...
    ctx:wait("CHANGE_TARGET_WAIT", 3.0, "ChangeTargetWait") -- BASERANGE.inc:336

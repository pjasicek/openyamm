...
script.labels["InitMonkHostility"] = function(ctx)
    ...
    ctx:self():addFriend("Player") -- MONKHOSTILITY.inc:17
    ctx:state().hostile_hMONK_HOSTILITY = ctx:objectOrNil("MONK_HOSTILITY") -- MONKHOSTILITY.inc:19
    if ctx:condition("hostile_hMONK_HOSTILITY!=0") then -- MONKHOSTILITY.inc:20
        ctx:self():link(ctx:object("hostile_hMONK_HOSTILITY")) -- MONKHOSTILITY.inc:21
        ctx:onEvent("OnObjectLinkBroken", "BecomeHostile") -- MONKHOSTILITY.inc:22
        ctx:onEvent("OnDamage", "_OnDamage") -- MONKHOSTILITY.inc:23
        ctx:onEvent("OnDeath", "_OnDeath") -- MONKHOSTILITY.inc:24
        ctx:onEvent("OnAlert", "_OnDamage") -- MONKHOSTILITY.inc:25
...
script.labels["BecomeHostile"] = function(ctx)
    ...
    ctx:state().hostile_hMONK_HOSTILITY = nil -- MONKHOSTILITY.inc:35
    ctx:addTrigger("use", "BlockRude") -- MONKHOSTILITY.inc:37
    ctx:self():addEnemy("Player") -- MONKHOSTILITY.inc:39
...
script.labels["_OnDamage"] = function(ctx)
    ...
    ctx:state().g_bTemp = ctx:object("g_hObject"):isPlayer() -- MONKHOSTILITY.inc:50
    if ctx:condition("g_bTemp==TRUE") then -- MONKHOSTILITY.inc:51
        ctx:onEvent("OnDamage", "DoNothing") -- MONKHOSTILITY.inc:52
        ctx:onEvent("OnDeath", "DoNothing") -- MONKHOSTILITY.inc:53
...
script.labels["_OnDeath"] = function(ctx)
    ...
    ctx:state().g_bTemp = ctx:object("g_hObject"):isPlayer() -- MONKHOSTILITY.inc:65
    if ctx:condition("g_bTemp==TRUE") then -- MONKHOSTILITY.inc:66
        ctx:onEvent("OnDamage", "DoNothing") -- MONKHOSTILITY.inc:67
        ctx:onEvent("OnDeath", "DoNothing") -- MONKHOSTILITY.inc:68
...
script.labels["TurnHostilityOn"] = function(ctx)
    ...
    if ctx:condition("hostile_hMONK_HOSTILITY!=0") then -- MONKHOSTILITY.inc:79
        ctx:playSound("sounds\\events\\alarmbell.wav", "DoNothing", 1, 5000, "FALSE", 100) -- MONKHOSTILITY.inc:80
        ctx:object("hostile_hMONK_HOSTILITY"):remove() -- MONKHOSTILITY.inc:81

...
script.labels["OnRude"] = function(ctx)
    ...
    if ctx:hasKey(330) then -- BANKORB.inc:34-35
        ...
        if ctx:hasKey("nKey") then -- BANKORB.inc:37-38
            ctx:playSound("sounds\\events\\quest.wav", "DoNothing", 100, 24000, "FALSE", 100) -- BANKORB.inc:39
...
script.labels["CheckAll"] = function(ctx)
    ...
    ctx:state().g_ncounter = 0 -- BANKORB.inc:57
    if ctx:hasKey(331) then -- BANKORB.inc:59-60
        ctx:state().g_ncounter = (tonumber(ctx:state().g_ncounter) or 0) + 1 -- BANKORB.inc:61
    end -- BANKORB.inc:62
    if ctx:hasKey(332) then -- BANKORB.inc:64-65
        ctx:state().g_ncounter = (tonumber(ctx:state().g_ncounter) or 0) + 1 -- BANKORB.inc:66
    end -- BANKORB.inc:67
    if ctx:hasKey(333) then -- BANKORB.inc:69-70
        ctx:state().g_ncounter = (tonumber(ctx:state().g_ncounter) or 0) + 1 -- BANKORB.inc:71
    end -- BANKORB.inc:72
    if ctx:hasKey(334) then -- BANKORB.inc:74-75
        ctx:state().g_ncounter = (tonumber(ctx:state().g_ncounter) or 0) + 1 -- BANKORB.inc:76
    end -- BANKORB.inc:77
    if ctx:hasKey(335) then -- BANKORB.inc:79-80
        ctx:state().g_ncounter = (tonumber(ctx:state().g_ncounter) or 0) + 1 -- BANKORB.inc:81
    end -- BANKORB.inc:82
    if ctx:hasKey(336) then -- BANKORB.inc:84-85
        ctx:state().g_ncounter = (tonumber(ctx:state().g_ncounter) or 0) + 1 -- BANKORB.inc:86
    end -- BANKORB.inc:87
    if ctx:condition("g_ncounter==6") then -- BANKORB.inc:89
        ...
        ctx:playSound("sounds\\events\\quest.wav", "DoNothing", 100, 240, "FALSE", 100) -- BANKORB.inc:93
...
script.labels["Init"] = function(ctx)
    ...
    ctx:state().nKey = 333 -- BANKORB.inc:112
    ctx:state().nKey2 = 327 -- BANKORB.inc:113

...
script.labels["AttackObstacle"] = function(ctx)
    ...
    if ctx:condition("g_hObject==g_hTarget") then -- BASEMELEE.inc:73
        ...
        if ctx:condition("g_bTemp==FALSE") then -- BASEMELEE.inc:76
            ctx:self():stop() -- BASEMELEE.inc:77
...
script.labels["PostAttack"] = function(ctx)
    ...
    if ctx:condition("g_bTemp==TRUE") then -- BASEMELEE.inc:107
        ...
        if ctx:condition("g_bTemp==FALSE") then -- BASEMELEE.inc:109
            ctx:randomInt(0, 100, "g_nRandom") -- BASEMELEE.inc:110
            if ctx:condition("g_nRandom < g_nStrafeAttackPct") then -- BASEMELEE.inc:111
                ctx:state().g_bPickDir = true -- BASEMELEE.inc:112
                ctx:self():setTarget(ctx:object("g_hTarget")) -- BASEMELEE.inc:113
                mm9.gosub(script, ctx, "BE_AttackStrafe") -- BASEMELEE.inc:114
                ctx:state().g_bPickDir = true -- BASEMELEE.inc:115
                ctx:state().g_bStrafeAttack = true -- BASEMELEE.inc:116
...
script.labels["ShouldEvade"] = function(ctx)
    ...
    ctx:randomInt(0, 100, "g_nRandom") -- BASEMELEE.inc:129
    ctx:state().g_nEvadeChance = ctx:self():getStat("EvadeChance") -- BASEMELEE.inc:131
    -- IsTurnBased g_bTemp
    -- if ( g_bTemp==TRUE )
    -- g_bTemp = FALSE
    -- exit
    ...
    if ctx:condition("g_nRandom < g_nEvadeChance") then -- BASEMELEE.inc:139
        ctx:state().g_bTemp = true -- BASEMELEE.inc:140
    else -- BASEMELEE.inc:141
        ctx:state().g_bTemp = false -- BASEMELEE.inc:142
...
script.labels["DoEvade"] = function(ctx)
    ...
    ctx:state().g_nEvadeTime = ctx:self():getStat("RecoveryTime") -- BASEMELEE.inc:163
    mm9.gosub(script, ctx, "SpeedThrottleStop") -- BASEMELEE.inc:164
    mm9.gosub(script, ctx, "AggressiveStop") -- BASEMELEE.inc:165
    ctx:onEvent("OnTargetBeyondDist", "g_nMaxEvadeDist", "CancelEvade") -- BASEMELEE.inc:166
    mm9.gosub(script, ctx, "BaseEvadeStart") -- BASEMELEE.inc:168
    -- Mul g_nTemp, 0.8
    ctx:set("g_nTemp", "g_nEvadeTime") -- BASEMELEE.inc:172
    ctx:state().g_nTemp = 2 -- BASEMELEE.inc:173
    if ctx:condition("g_nTemp < 1") then -- BASEMELEE.inc:174
        ctx:state().g_nTemp = 1 -- BASEMELEE.inc:175
    end -- BASEMELEE.inc:176
    ctx:wait("AGGRESSIVE_WAIT", "g_nTemp", "CancelEvade") -- BASEMELEE.inc:179
...
script.labels["AttackDone"] = function(ctx)
    ...
    if ctx:condition("g_bTemp==FALSE") then -- BASEMELEE.inc:225
        ctx:state().g_bEvading = false -- BASEMELEE.inc:226
        ...
        if ctx:condition("g_bStrafeAttack==TRUE") then -- BASEMELEE.inc:231
            ...
            ctx:wait("AGGRESSIVE_WAIT", 0.3, "AttackBackpedalDone") -- BASEMELEE.inc:233
...
script.labels["BaseInit"] = function(ctx)
    ...
    ctx:state().g_nStrafeAttackPct = ctx:self():getStat("StrafeAttackPct") -- BASEMELEE.inc:268
    ctx:state().g_bCanBlendAnim = ctx:self():getStat("CanBlendAnim") -- BASEMELEE.inc:269
    ctx:state().g_bCanHeadTurn = ctx:self():getStat("CanHeadTurn") -- BASEMELEE.inc:270

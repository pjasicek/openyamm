...
script.labels["WanderObstacle"] = function(ctx)
    ...
    if ctx:condition("g_IsRunning == TRUE") then -- WANDER.inc:69
        ...
        if ctx:condition("g_bStoppedAtObstacle == TRUE") then -- WANDER.inc:74
            ctx:command("set", "g_rotX, g_StoppedRotX") -- WANDER.inc:75
            ctx:command("set", "g_rotY, g_StoppedRotY") -- WANDER.inc:76
            ctx:command("set", "g_rotZ, g_StoppedRotZ") -- WANDER.inc:77
            do return mm9.gotoLabel(script, ctx, "GoAroundObstacle") end -- WANDER.inc:78
        end -- WANDER.inc:79
        ctx:command("getrandomint", "1, 10, g_nRandom") -- WANDER.inc:81
        if ctx:condition("g_nRandom > g_nStopAtObstacle") then -- WANDER.inc:82
            ...
            ctx:command("set", "g_bStoppedAtObstacle, TRUE") -- WANDER.inc:91
            ctx:command("stop", "") -- WANDER.inc:93
        end -- WANDER.inc:94
    end -- WANDER.inc:95
end
...
script.labels["GoAroundObstacle"] = function(ctx)
    -- WANDER.inc:98
    ctx:command("set", "g_bStoppedAtObstacle, FALSE") -- WANDER.inc:99
    ctx:getParam(1, "g_rotX") -- WANDER.inc:101
    ctx:getParam(2, "g_rotY") -- WANDER.inc:102
    ctx:getParam(3, "g_rotZ") -- WANDER.inc:103
    ctx:command("getrandomfloat", "0.2, 0.6, g_nRandom") -- WANDER.inc:105
    ctx:command("getrandomint", "0,1, g_nTemp") -- WANDER.inc:106
    if ctx:condition("g_nTemp==1") then -- WANDER.inc:108
        ctx:command("mul", "g_nRandom, -1") -- WANDER.inc:109
    end -- WANDER.inc:110
    ctx:command("add", "g_rotX, g_nRandom") -- WANDER.inc:112
    ctx:command("getrandomfloat", "0.2, 0.6, g_nRandom") -- WANDER.inc:114
    ctx:command("getrandomint", "0,1, g_nTemp") -- WANDER.inc:115
    if ctx:condition("g_nTemp==1") then -- WANDER.inc:117
        ctx:command("mul", "g_nRandom, -1") -- WANDER.inc:118
    end -- WANDER.inc:119
    ctx:command("add", "g_rotZ, g_nRandom") -- WANDER.inc:121
    ctx:command("facedir", "g_rotX, g_rotY, g_rotZ, g_wanderObstacleRotRate") -- WANDER.inc:123
    if ctx:condition("g_IsRunning == FALSE") then -- WANDER.inc:125
        ctx:command("walk", "") -- WANDER.inc:126
    else -- WANDER.inc:127
        ctx:command("run", "") -- WANDER.inc:128
    end -- WANDER.inc:129
    -- Setup next tick
    ctx:command("getrandomfloat", "g_IdleCheckMin, g_IdleCheckMax, g_nRandom") -- WANDER.inc:133
    ctx:command("wait", "WANDER_WAIT_NBR, g_nRandom, WanderTick") -- WANDER.inc:134
...
script.labels["WanderGo"] = function(ctx)
    ...
    ctx:command("getrandomfloat", "g_TurnDegreeMin, g_TurnDegreeMax, g_turnDeg") -- WANDER.inc:145
    -- GetRandomFloat 20, 50, g_turnRate
    -- Set g_turnRate, g_turnDeg
    if ctx:condition("g_IsRunning == FALSE") then -- WANDER.inc:150
        ctx:command("set", "g_turnRate, 10") -- WANDER.inc:151
    else -- WANDER.inc:152
        ctx:command("set", "g_turnRate, 45") -- WANDER.inc:153
    end -- WANDER.inc:154
    ctx:command("getpos", "g_hMyObject, g_posX, g_posY, g_posZ") -- WANDER.inc:156
    ctx:command("calcdist", "g_posX,g_posY,g_posZ,g_startX,g_startY,g_startZ,g_dist") -- WANDER.inc:158
    if ctx:condition("g_dist>MAX_DIST_FROM_STARTPOINT") then -- WANDER.inc:160
        -- Too far away from original starting point!!
        ctx:command("facepos", "g_startX, g_startY, g_startZ, 180") -- WANDER.inc:162
    else -- WANDER.inc:163
        ctx:command("getrandomint", "0,1, g_nRandom") -- WANDER.inc:164
        if ctx:condition("g_nRandom==0") then -- WANDER.inc:166
            ctx:command("mul", "g_turnDeg, -1") -- WANDER.inc:167
        end -- WANDER.inc:168
        ctx:command("rotate", "0,1,0,g_turnDeg,g_turnRate") -- WANDER.inc:170
    end -- WANDER.inc:171
    if ctx:condition("g_IsRunning == FALSE") then -- WANDER.inc:173
        ctx:command("walk", "") -- WANDER.inc:174
    else -- WANDER.inc:175
        ctx:command("run", "") -- WANDER.inc:176
    end -- WANDER.inc:177
    ctx:command("getrandomfloat", "g_IdleCheckMin, g_IdleCheckMax, g_nRandom") -- WANDER.inc:179
    ctx:command("wait", "WANDER_WAIT_NBR, g_nRandom, WanderTick") -- WANDER.inc:180
...
script.labels["WanderPause"] = function(ctx)
    ...
    if ctx:condition("g_IsAttacking == FALSE") then -- WANDER.inc:187
        if ctx:condition("g_IsRunning == FALSE") then -- WANDER.inc:188
            ctx:command("getrandomint", "1, 10, g_nRandom") -- WANDER.inc:189
            if ctx:condition("g_nRandom <= g_SpecialAnimFrequency") then -- WANDER.inc:190
                ctx:trigger("g_hMyObject", "SpecialAnim") -- WANDER.inc:191
            else -- WANDER.inc:192
                ctx:command("setidle", "") -- WANDER.inc:193
            end -- WANDER.inc:194
        end -- WANDER.inc:195
    end -- WANDER.inc:196
    ctx:command("getrandomfloat", "g_IdleCheckMin, g_IdleCheckMax, g_nRandom") -- WANDER.inc:198
    ctx:command("wait", "WANDER_WAIT_NBR, g_nRandom, WanderTick") -- WANDER.inc:199
...
script.labels["WanderTick"] = function(ctx)
    ...
    if ctx:condition("g_IsRunning == TRUE") then -- WANDER.inc:206
        ctx:command("gettime", "g_CurrTime") -- WANDER.inc:207
        ...
        ctx:command("getrandomint", "0, 10, g_nRandom") -- WANDER.inc:217
...
script.labels["WanderStartup"] = function(ctx)
    -- WANDER.inc:235
    ctx:command("getpos", "g_hMyObject, g_startX, g_startY, g_startZ") -- WANDER.inc:238
...
script.labels["WanderInit"] = function(ctx)
    -- WANDER.inc:245
    ctx:command("getmyhandle", "g_hMyObject") -- WANDER.inc:248
    ctx:command("onobstacle", "WanderObstacle") -- WANDER.inc:249
    ctx:command("onstuckdone", "WanderStuckDone") -- WANDER.inc:250
    -- Must wait and do some stuff after initial execution...
    ctx:command("wait", "WANDER_WAIT_NBR, 0.1, WanderStartup") -- WANDER.inc:253

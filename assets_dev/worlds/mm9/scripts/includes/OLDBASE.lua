...
script.labels["BaseChaseTick"] = function(ctx)
    ...
    ctx:state().g_nDist1 = ctx:self():distanceTo(ctx:object("g_hTarget")) -- OLDBASE.inc:36
    if ctx:condition("g_bRunning==TRUE") then -- OLDBASE.inc:38
        if ctx:condition("g_nDist1 < 120.0") then -- OLDBASE.inc:39
            ctx:set("g_sOut", "g_nDist1") -- OLDBASE.inc:40
            ctx:debugOut("g_sOut") -- OLDBASE.inc:41
            -- Time to Walk!
            ctx:state().g_bRunning = false -- OLDBASE.inc:43
            ctx:self():walkTo(ctx:object("g_hTarget")) -- OLDBASE.inc:44
        end -- OLDBASE.inc:45
    else -- OLDBASE.inc:46
        if ctx:condition("g_nDist1 > 200.0") then -- OLDBASE.inc:47
            ctx:set("g_sOut", "g_nDist1") -- OLDBASE.inc:48
            ctx:state().g_bRunning = true -- OLDBASE.inc:49
            ctx:self():runTo(ctx:object("g_hTarget")) -- OLDBASE.inc:50
        end -- OLDBASE.inc:51
    end -- OLDBASE.inc:52
    ctx:wait(0.1, 0.1, "BaseChaseTick") -- OLDBASE.inc:54
    do return ctx:exit("") end -- OLDBASE.inc:56
end

script.labels["BaseGoGetHim"] = function(ctx)
    -- OLDBASE.inc:59
    ctx:self():setTarget(ctx:object("g_hTarget")) -- OLDBASE.inc:61
    -- Run after him. Don't need a callback because
    -- we have OnAttackReady call BaseAttackReady
    -- when we are close enough...
    ctx:state().g_bRunning = true -- OLDBASE.inc:69
    ctx:state().g_bChasing = true -- OLDBASE.inc:70
    ctx:self():runTo(ctx:object("g_hTarget")) -- OLDBASE.inc:71
    ctx:wait(0.1, 0.1, "BaseChaseTick") -- OLDBASE.inc:73
    do return ctx:exit(1) end -- OLDBASE.inc:75
end

script.labels["BaseRotationDone"] = function(ctx)
    -- OLDBASE.inc:78
    ctx:self():setTarget(ctx:object("g_hTarget")) -- OLDBASE.inc:80
    ctx:self():playAnimationCommand("Threat", "FALSE", "BaseGoGetHim") -- OLDBASE.inc:81
...
script.labels["BaseFoundPlayer"] = function(ctx)
    ...
    ctx:state().g_hTarget = ctx:player() -- OLDBASE.inc:95
    ...
    ctx:state().g_bInAttackRange = 0 -- OLDBASE.inc:104
    -- Turn and face target before running after him!
    ctx:self():faceObject(ctx:player(), 360, "BaseRotationDone") -- OLDBASE.inc:107
...
script.labels["BaseAttackReady"] = function(ctx)
    ...
    ctx:state().g_bChasing = false -- OLDBASE.inc:119
    ...
    -- will get called...
    ctx:state().g_bInAttackRange = 1 -- OLDBASE.inc:127
    ctx:self():attack() -- OLDBASE.inc:129
...
script.labels["BaseTargetOutOfRange"] = function(ctx)
    ...
    -- Go after him!
    ctx:debugOut("Base", "Target", "Out", "Of", "Range") -- OLDBASE.inc:141
    ctx:state().g_bInAttackRange = 0 -- OLDBASE.inc:143
...
script.labels["BaseLostTarget"] = function(ctx)
    ...
    ctx:state().g_hTarget = 0 -- OLDBASE.inc:155
    ctx:self():setIdle() -- OLDBASE.inc:156
...
script.labels["BaseDamageDone"] = function(ctx)
    ...
    if ctx:condition("g_hTarget==0") then -- OLDBASE.inc:182
        ctx:set("g_hTarget", "g_hAttacker") -- OLDBASE.inc:183
        ...
        if ctx:condition("g_hAttacker!=0") then -- OLDBASE.inc:185
            if ctx:condition("g_hAttacker!=g_hTarget") then -- OLDBASE.inc:186
                ...
                ctx:state().g_nTemp = 70 -- OLDBASE.inc:188
                if ctx:condition("g_bInAttackRange==1") then -- OLDBASE.inc:189
                    ...
                    ctx:state().g_nTemp = 10 -- OLDBASE.inc:192
                end -- OLDBASE.inc:193
                ctx:state().g_nDist1 = ctx:self():distanceTo(ctx:object("g_hTarget")) -- OLDBASE.inc:195
                ctx:state().g_nDist2 = ctx:self():distanceTo(ctx:object("g_hAttacker")) -- OLDBASE.inc:196
                if ctx:condition("g_nDist2 > g_nDist1") then -- OLDBASE.inc:198
                    -- reduce odds even further...
                    ctx:state().g_nTemp = (tonumber(ctx:state().g_nTemp) or 0) / 2 -- OLDBASE.inc:200
                end -- OLDBASE.inc:201
                ctx:randomInt(0, 100, "g_nRandom") -- OLDBASE.inc:203
                if ctx:condition("g_nRandom < g_nTemp") then -- OLDBASE.inc:204
                    -- Okay, we decided to switch our current target to the
                    -- attacker!
                    ctx:set("g_hTarget", "g_hAttacker") -- OLDBASE.inc:209
                end -- OLDBASE.inc:210
            end -- OLDBASE.inc:211
        end -- OLDBASE.inc:212
    end -- OLDBASE.inc:213
...
script.labels["InitBase"] = function(ctx)
    ...
    ctx:onEvent("OnFoundPlayer", "BaseFoundPlayer") -- OLDBASE.inc:252
    ctx:onEvent("OnLostTarget", "BaseLostTarget") -- OLDBASE.inc:253
    ctx:onEvent("OnTargetOutOfRange", "BaseTargetOutOfRange") -- OLDBASE.inc:254
    ctx:onEvent("OnAttackReady", "BaseAttackReady") -- OLDBASE.inc:255
    ctx:onEvent("OnStuckDone", "BaseStuckDone") -- OLDBASE.inc:256
    ctx:onEvent("OnDamageDone", "BaseDamageDone") -- OLDBASE.inc:257
    ctx:onEvent("OnDamage", "BaseDamage") -- OLDBASE.inc:258

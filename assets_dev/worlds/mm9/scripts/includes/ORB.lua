...
script.labels["reward"] = function(ctx)
    ...
    if not ctx:hasKey(330) then -- ORB.inc:27-28
        ...
        ctx:state().BeenDone = 1 -- ORB.inc:31
...
script.labels["Orb"] = function(ctx)
    -- ORB.inc:37
    ctx:state().NoDice = 0 -- ORB.inc:40
    if not ctx:hasKey(330) then -- ORB.inc:42-43
        if ctx:hasKey(323) then -- ORB.inc:45-46
            ctx:arrayPut("OrbArray", 0, 1) -- ORB.inc:47
        else -- ORB.inc:48
            ctx:state().NoDice = (tonumber(ctx:state().NoDice) or 0) + 1 -- ORB.inc:49
        end -- ORB.inc:50
        if ctx:hasKey(324) then -- ORB.inc:52-53
            ctx:arrayPut("OrbArray", 1, 1) -- ORB.inc:54
        else -- ORB.inc:55
            ctx:state().NoDice = (tonumber(ctx:state().NoDice) or 0) + 1 -- ORB.inc:56
        end -- ORB.inc:57
        if ctx:hasKey(325) then -- ORB.inc:59-60
            ctx:arrayPut("OrbArray", 2, 1) -- ORB.inc:61
        else -- ORB.inc:62
            ctx:state().NoDice = (tonumber(ctx:state().NoDice) or 0) + 1 -- ORB.inc:63
        end -- ORB.inc:64
        if ctx:hasKey(326) then -- ORB.inc:66-67
            ctx:arrayPut("OrbArray", 3, 1) -- ORB.inc:68
        else -- ORB.inc:69
            ctx:state().NoDice = (tonumber(ctx:state().NoDice) or 0) + 1 -- ORB.inc:70
        end -- ORB.inc:71
        if ctx:hasKey(327) then -- ORB.inc:73-74
            ctx:arrayPut("OrbArray", 4, 1) -- ORB.inc:75
        else -- ORB.inc:76
            ctx:state().NoDice = (tonumber(ctx:state().NoDice) or 0) + 1 -- ORB.inc:77
        end -- ORB.inc:78
        if ctx:hasKey(328) then -- ORB.inc:80-81
            ctx:arrayPut("OrbArray", 5, 1) -- ORB.inc:82
        else -- ORB.inc:83
            ctx:state().NoDice = (tonumber(ctx:state().NoDice) or 0) + 1 -- ORB.inc:84
        end -- ORB.inc:85
        if ctx:hasKey(329) then -- ORB.inc:87-88
            ctx:arrayPut("OrbArray", 6, 1) -- ORB.inc:89
        else -- ORB.inc:90
            ctx:state().NoDice = (tonumber(ctx:state().NoDice) or 0) + 1 -- ORB.inc:91
        end -- ORB.inc:92
    end -- ORB.inc:93
...
script.labels["CheckAllorb"] = function(ctx)
    ...
    ctx:state().counter = 0 -- ORB.inc:111
end

script.labels["CheckAllOrbloop"] = function(ctx)
    -- ORB.inc:116
    ctx:arrayGet("OrbArray", "counter", "GiveOrb") -- ORB.inc:121
    if ctx:condition("GiveOrb==false") then -- ORB.inc:122
    end -- ORB.inc:123
    ctx:state().Counter = (tonumber(ctx:state().Counter) or 0) + 1 -- ORB.inc:125

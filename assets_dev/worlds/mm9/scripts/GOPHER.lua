...
script.labels["OnSpecial"] = function(ctx)
    -- GOPHER.scr:18
    ctx:command("getrandomint", "0, 10, g_nRandom") -- GOPHER.scr:20
    if ctx:condition("g_nRandom < 6") then -- GOPHER.scr:21
        ctx:command("playanim", "up, GopherUpDone") -- GOPHER.scr:22
    else -- GOPHER.scr:23
        -- We put the gopher down here so we can peek out
        ctx:command("playanim", "down, GopherDownDone") -- GOPHER.scr:25
    end -- GOPHER.scr:26
    ctx:command("wait", "1, WanderTick") -- GOPHER.scr:28
    do return ctx:exit("") end -- GOPHER.scr:31
end

script.labels["GopherUpDone"] = function(ctx)
    -- GOPHER.scr:34
    ctx:command("playanim", "down") -- GOPHER.scr:36
    ctx:command("wait", "2, WanderTick") -- GOPHER.scr:38
    do return ctx:exit("") end -- GOPHER.scr:40
end

script.labels["GopherDownDone"] = function(ctx)
    -- GOPHER.scr:43
    ctx:command("playanim", "peek") -- GOPHER.scr:45
    ctx:command("wait", "2, WanderTick") -- GOPHER.scr:47
...
script.labels["Main"] = function(ctx)
    ...
    if ctx:condition("g_nDistance != 0") then -- GOPHER.scr:65
        ctx:command("set", "MAX_DIST_FROM_STARTPOINT, g_nDistance") -- GOPHER.scr:66
    else -- GOPHER.scr:67
        ctx:command("set", "MAX_DIST_FROM_STARTPOINT, 350") -- GOPHER.scr:68
    end -- GOPHER.scr:69
    -- Set how often we idle 1 equals 10% valid numbers are 1 to 10
    ctx:command("set", "g_IdleFrequency, 10") -- GOPHER.scr:72
    -- Set max time in seconds before idle check
    ctx:command("set", "g_IdleCheckMin, 10") -- GOPHER.scr:75
    ctx:command("set", "g_IdleCheckMax, 10") -- GOPHER.scr:76
    -- Set how often special anim is played valid numbers are 1 to 10
    ctx:command("set", "g_SpecialAnimFrequency, 8") -- GOPHER.scr:79

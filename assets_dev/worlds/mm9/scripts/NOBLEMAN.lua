...
script.labels["NoblemanAlert"] = function(ctx)
    ...
    ctx:command("getclassname", "hAlertedBy, sAlertName") -- NOBLEMAN.scr:27
...
script.labels["NoblemanAttackReady"] = function(ctx)
    ...
    ctx:command("getclassname", "g_hTarget, sAlertName") -- NOBLEMAN.scr:64
    if ctx:condition("sAlertName == TownsFolkBoy") then -- NOBLEMAN.scr:65
        ctx:command("set", "g_bOkAttackType, 1") -- NOBLEMAN.scr:66
    end -- NOBLEMAN.scr:67
    if ctx:condition("sAlertName == TownsFolkGirl") then -- NOBLEMAN.scr:69
        ctx:command("set", "g_bOkAttackType, 1") -- NOBLEMAN.scr:70
    end -- NOBLEMAN.scr:71
    if ctx:condition("sAlertName == TownsFolkMale") then -- NOBLEMAN.scr:73
        ctx:command("set", "g_bOkAttackType, 1") -- NOBLEMAN.scr:74
    end -- NOBLEMAN.scr:75
    if ctx:condition("sAlertName == TownsFolkFemale") then -- NOBLEMAN.scr:77
        ctx:command("set", "g_bOkAttackType, 1") -- NOBLEMAN.scr:78
    end -- NOBLEMAN.scr:79
    if ctx:condition("sAlertName == TownsFolkFemale") then -- NOBLEMAN.scr:81
        ctx:command("set", "g_bOkAttackType, 1") -- NOBLEMAN.scr:82
    end -- NOBLEMAN.scr:83
    if ctx:condition("sAlertName == Nobleman") then -- NOBLEMAN.scr:85
        ctx:command("set", "g_bOkAttackType, 1") -- NOBLEMAN.scr:86
    end -- NOBLEMAN.scr:87
    ctx:command("set", "g_bFighting, TRUE") -- NOBLEMAN.scr:89
    ctx:command("gettime", "g_nLastAttackTime") -- NOBLEMAN.scr:91
    ctx:command("attack", "") -- NOBLEMAN.scr:93
...
script.labels["InitNobleman"] = function(ctx)
    ...
    if ctx:condition("g_nDistance != 0") then -- NOBLEMAN.scr:104
        ctx:command("set", "MAX_DIST_FROM_STARTPOINT, g_nDistance") -- NOBLEMAN.scr:105
    else -- NOBLEMAN.scr:106
        ctx:command("set", "MAX_DIST_FROM_STARTPOINT, 700") -- NOBLEMAN.scr:107
    end -- NOBLEMAN.scr:108
    -- Set how often hen is idle 1 equals 10% valid numbers are 1 to 10
    ctx:command("set", "g_IdleFrequency, 4") -- NOBLEMAN.scr:111
    -- Set max time in seconds before idle check
    ctx:command("set", "g_IdleCheckMin, 7") -- NOBLEMAN.scr:114
    ctx:command("set", "g_IdleCheckMax, 7") -- NOBLEMAN.scr:115
    -- Set how often special anim is played valid numbers are 1 to 10
    ctx:command("set", "g_SpecialAnimFrequency, 0") -- NOBLEMAN.scr:118
...
script.labels["Main"] = function(ctx)
    ...
    ctx:command("onalert", "NoblemanAlert") -- NOBLEMAN.scr:136
    ctx:command("onattackready", "NoblemanAttackReady") -- NOBLEMAN.scr:137
    -- Monitoring this with no function will keep the AI from looking for players
    ctx:command("onfoundplayer", "") -- NOBLEMAN.scr:140

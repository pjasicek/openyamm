...
script.labels["Tick"] = function(ctx)
    -- FOLLOWCAM.scr:15
    ctx:state().g_posX, ctx:state().g_posY, ctx:state().g_posZ = ctx:object("hFollowObj"):pos() -- FOLLOWCAM.scr:16
    ctx:add("g_posY", "nHeight") -- FOLLOWCAM.scr:18
    ctx:self():setPos("g_posX", "g_posY", "g_posZ") -- FOLLOWCAM.scr:20
    ctx:wait(0.01, 0.01, "Tick") -- FOLLOWCAM.scr:22
...
script.labels["TurnOn"] = function(ctx)
    ...
    if ctx:condition("g_nTemp > 0") then -- FOLLOWCAM.scr:34
        ctx:set("nHeight", "g_nTemp") -- FOLLOWCAM.scr:35
    end -- FOLLOWCAM.scr:36
    ctx:state().hFollowObj = ctx:objectOrNil("sObjName") -- FOLLOWCAM.scr:38
    if ctx:condition("hFollowObj==NULL") then -- FOLLOWCAM.scr:40
        ctx:set("g_sOut", "sObjName") -- FOLLOWCAM.scr:41
        ctx:add("g_sOut", "<--- is an invalid object name...") -- FOLLOWCAM.scr:42
        ctx:debugOut("g_sOut") -- FOLLOWCAM.scr:43
        do return ctx:exit("TRUE") end -- FOLLOWCAM.scr:44
    end -- FOLLOWCAM.scr:45
    mm9.gosub(script, ctx, "Tick") -- FOLLOWCAM.scr:47
    ctx:traceOff() -- FOLLOWCAM.scr:49
    do return ctx:exit("FALSE") end -- FOLLOWCAM.scr:51
end

script.labels["TurnOff"] = function(ctx)
    -- FOLLOWCAM.scr:53
    ctx:wait(0, 0, "Dummy") -- FOLLOWCAM.scr:54

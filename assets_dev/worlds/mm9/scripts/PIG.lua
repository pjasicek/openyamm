...
script.labels["OnSpecial"] = function(ctx)
    ...
    if ctx:condition("g_nRandom < 3") then -- PIG.scr:21
        ...
        ctx:self():playAnimation("Standdown", "OnStandDownRestDone") -- PIG.scr:23
    else -- PIG.scr:24
        ctx:self():playAnimation("Standdown", "OnStandDownRollDone") -- PIG.scr:25
    end -- PIG.scr:26
...
script.labels["OnStandDownRestDone"] = function(ctx)
    -- PIG.scr:31
    ctx:randomInt(9, 15, "g_nRandom") -- PIG.scr:33
    ctx:self():loopAnimation("Rest", "g_nRandom", "OnDownDone") -- PIG.scr:34
    ctx:wait(0, 0, "DoNothing") -- PIG.scr:36
    do return ctx:exit("") end -- PIG.scr:38
end

script.labels["OnStandDownRollDone"] = function(ctx)
    -- PIG.scr:41
    ctx:self():loopAnimation("Roll", 4, "OnDownRoll2Done") -- PIG.scr:43
    ctx:wait(0, 0, "DoNothing") -- PIG.scr:45
    do return ctx:exit("") end -- PIG.scr:47
end

script.labels["OnDownRoll2Done"] = function(ctx)
    -- PIG.scr:50
    ctx:randomInt(9, 15, "g_nRandom") -- PIG.scr:52
    ctx:self():loopAnimation("Rest", "g_nRandom", "OnDownDone") -- PIG.scr:53
    ctx:wait(0, 0, "DoNothing") -- PIG.scr:55
    do return ctx:exit("") end -- PIG.scr:57
end

script.labels["OnDownDone"] = function(ctx)
    -- PIG.scr:60
    ctx:self():playAnimation("Standup") -- PIG.scr:62
    ctx:wait(0, 1, "WanderTick") -- PIG.scr:64
...
script.labels["OnRun"] = function(ctx)
    -- PIG.scr:77
    ctx:state().g_posX, ctx:state().g_posY, ctx:state().g_posZ = ctx:object("hAttacker"):rotation() -- PIG.scr:80
    ctx:self():faceDir("g_PosX", "g_PosY", "g_PosZ") -- PIG.scr:82
    ctx:self():run() -- PIG.scr:84
    ctx:state().g_IsRunning = true -- PIG.scr:86
    -- This will cause a more erratic runniing behavior
    ctx:state().g_IdleCheckMin = 1 -- PIG.scr:88
    ctx:state().g_IdleCheckMax = 1 -- PIG.scr:89
...
script.labels["OnRunning"] = function(ctx)
    ...
    ctx:self():sendAlert(ctx:object("hAttacker")) -- PIG.scr:101
...
script.labels["OnStopRunning"] = function(ctx)
    ...
    ctx:state().g_IdleCheckMin = 7 -- PIG.scr:110
    ctx:state().g_IdleCheckMax = 7 -- PIG.scr:111
    ctx:state().g_TurnDegreeMin = 15 -- PIG.scr:113
    ctx:state().g_TurnDegreeMax = 90 -- PIG.scr:114
    ctx:state().g_IsRunning = false -- PIG.scr:116
...
script.labels["PigOnDamage"] = function(ctx)
    ...
    -- Alert nearby AI
    ctx:self():sendAlert(ctx:object("hAttacker")) -- PIG.scr:130
    ctx:getTime("g_StopRunTime") -- PIG.scr:132
    ctx:state().g_StopRunTime = (tonumber(ctx:state().g_StopRunTime) or 0) + 15 -- PIG.scr:133
...
script.labels["InitPig"] = function(ctx)
    ...
    if ctx:condition("g_nDistance != 0") then -- PIG.scr:155
        ctx:set("MAX_DIST_FROM_STARTPOINT", "g_nDistance") -- PIG.scr:156
    else -- PIG.scr:157
        ctx:state().MAX_DIST_FROM_STARTPOINT = 350 -- PIG.scr:158
    end -- PIG.scr:159
    -- Set how often we stop at obstacle
    ctx:state().g_nStopAtObstacle = 1 -- PIG.scr:162
    -- Set how often hen is idle 1 equals 10%
    ctx:state().g_IdleFrequency = 5 -- PIG.scr:165
    -- Set how often special anim is played valid numbers are 1 to 10
    ctx:state().g_SpecialAnimFrequency = 8 -- PIG.scr:168
    -- Set max time before idle check
    ctx:state().g_IdleCheckMin = 8 -- PIG.scr:171
    ctx:state().g_IdleCheckMax = 8 -- PIG.scr:172
...
script.labels["Main"] = function(ctx)
    ...
    ctx:onEvent("OnDamageDone", "PigOnDamageDone") -- PIG.scr:186
    ctx:onEvent("OnDamage", "PigOnDamage") -- PIG.scr:187

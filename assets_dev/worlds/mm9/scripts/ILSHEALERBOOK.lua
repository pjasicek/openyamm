...
script.labels["Onclickon"] = function(ctx)
    ...
    if ctx:condition("counter==0") then -- ILSHEALERBOOK.scr:23
        ctx:self():playAnimation("OpenBook") -- ILSHEALERBOOK.scr:25
        ctx:state().counter = (tonumber(ctx:state().counter) or 0) + 1 -- ILSHEALERBOOK.scr:26
        do return ctx:exit("") end -- ILSHEALERBOOK.scr:27
    end -- ILSHEALERBOOK.scr:28
    if ctx:condition("counter==1") then -- ILSHEALERBOOK.scr:30
        ctx:self():playAnimation("TurnPage") -- ILSHEALERBOOK.scr:31
        ctx:state().counter = (tonumber(ctx:state().counter) or 0) + 1 -- ILSHEALERBOOK.scr:32
    ...
    if ctx:condition("counter==2") then -- ILSHEALERBOOK.scr:37
        ctx:self():playAnimation("CloseBook") -- ILSHEALERBOOK.scr:38
        ctx:state().counter = 0 -- ILSHEALERBOOK.scr:39

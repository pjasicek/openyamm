...
script.labels["OnSpecial"] = function(ctx)
    ...
    ctx:randomInt(1, 10, "g_nRandom") -- FARMERFEMALE.scr:33
    if ctx:condition("g_nRandom <= 5") then -- FARMERFEMALE.scr:34
        ctx:self():playAnimation("feedchicken1", "OnFeedChickenDone") -- FARMERFEMALE.scr:35
    else -- FARMERFEMALE.scr:36
        ctx:self():playAnimation("feedchicken2", "OnFeedChickenDone") -- FARMERFEMALE.scr:37
...
script.labels["FarmerFeedChickens"] = function(ctx)
    -- FARMERFEMALE.scr:48
    ctx:getObjects("Hen", 500, 50, "g_hHenArray", "g_nObjects") -- FARMERFEMALE.scr:50
end

script.labels["GetHens"] = function(ctx)
    -- FARMERFEMALE.scr:52
    if ctx:condition("g_nCtr < g_nObjects") then -- FARMERFEMALE.scr:53
        ctx:arrayGet("g_hHenArray", "g_nCtr", "g_hCurrHen") -- FARMERFEMALE.scr:54
        ctx:state().g_nCtr = (tonumber(ctx:state().g_nCtr) or 0) + 1 -- FARMERFEMALE.scr:55
...
script.labels["OnFeedChickenDone"] = function(ctx)
    -- FARMERFEMALE.scr:66
    ctx:wait(1, 1, "WanderTick") -- FARMERFEMALE.scr:68
...
script.labels["Main"] = function(ctx)
    ...
    if ctx:condition("g_nDistance != 0") then -- FARMERFEMALE.scr:87
        ctx:set("MAX_DIST_FROM_STARTPOINT", "g_nDistance") -- FARMERFEMALE.scr:88
    else -- FARMERFEMALE.scr:89
        ctx:state().MAX_DIST_FROM_STARTPOINT = 350 -- FARMERFEMALE.scr:90
    end -- FARMERFEMALE.scr:91
    -- Set how often hen is idle 1 equals 10% valid numbers are 1 to 10
    ctx:state().g_IdleFrequency = 10 -- FARMERFEMALE.scr:94
    -- Set max time in seconds before idle check
    ctx:state().g_IdleCheckMin = 2 -- FARMERFEMALE.scr:97
    ctx:state().g_IdleCheckMax = 6 -- FARMERFEMALE.scr:98
    -- Set how often special anim is played valid numbers are 1 to 10
    ctx:state().g_SpeicalAnimFrequency = 5 -- FARMERFEMALE.scr:101

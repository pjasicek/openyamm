...
script.labels["Main"] = function(ctx)
    ...
    ctx:onEvent("OnCacheFiles", "CacheFiles") -- CHASM_GHOSTSPAWNER.scr:32
    ctx:set("LISTINDEX", "LISTLAST") -- CHASM_GHOSTSPAWNER.scr:34
...
script.labels["CacheFiles"] = function(ctx)
    ...
    ctx:cacheScript("ghost.scr") -- CHASM_GHOSTSPAWNER.scr:43
    ctx:cacheClientFx("SPELL_BLUEFIRE") -- CHASM_GHOSTSPAWNER.scr:45
...
script.labels["SpawnCreature"] = function(ctx)
    ...
    ctx:state().x, ctx:state().y, ctx:state().z = ctx:object("LISTOBJECT"):pos() -- CHASM_GHOSTSPAWNER.scr:54
    ctx:state().hDummy = ctx:spawn("x", "y", "z", "SPAWN_PARAM") -- CHASM_GHOSTSPAWNER.scr:55
    if ctx:condition("hDummy!=0") then -- CHASM_GHOSTSPAWNER.scr:57
        ctx:object("hDummy"):doClientFx("SPELL_BLUEFIRE", "FALSE", "TRUE") -- CHASM_GHOSTSPAWNER.scr:58
    end -- CHASM_GHOSTSPAWNER.scr:59
    if ctx:condition("LISTINDEX==LISTLAST") then -- CHASM_GHOSTSPAWNER.scr:61
        ctx:removeTrigger("spawn") -- CHASM_GHOSTSPAWNER.scr:62

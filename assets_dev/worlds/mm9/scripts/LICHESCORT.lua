...
script.labels["WalkToEngine"] = function(ctx)
    -- LICHESCORT.scr:25
    ctx:state().hDestination = ctx:objectOrNil("sDestinationName") -- LICHESCORT.scr:27
    if ctx:condition("hDestination!=0") then -- LICHESCORT.scr:28
        ctx:self():walkTo(ctx:object("hDestination"), 10, "StopMoving") -- LICHESCORT.scr:29
        ctx:wait(0, 10, "EndScene") -- LICHESCORT.scr:30
...
script.labels["PlayAnim"] = function(ctx)
    -- LICHESCORT.scr:36
    ctx:self():doClientFx("SPELL_TRANSFUSION", "FALSE", "TRUE") -- LICHESCORT.scr:38
    ctx:self():playAnimation("resurrectspell", "EndScene") -- LICHESCORT.scr:40

...
script.labels["OnAgree"] = function(ctx)
    -- ARG_FORAD.scr:12
    ctx:command("playanim", "Agree Init") -- ARG_FORAD.scr:14
    do return ctx:exit("") end -- ARG_FORAD.scr:15
end

script.labels["OnApplause"] = function(ctx)
    -- ARG_FORAD.scr:17
    ctx:command("playanim", "Applause Init") -- ARG_FORAD.scr:20
...
script.labels["OnDone"] = function(ctx)
    ...
    ctx:command("getobjecthandle", "Argueman g_hobject") -- ARG_FORAD.scr:27
    ctx:trigger("g_hobject", "Done") -- ARG_FORAD.scr:28
    do return ctx:exit("") end -- ARG_FORAD.scr:29
end

script.labels["OnSpeak6"] = function(ctx)
    -- ARG_FORAD.scr:32
    ctx:command("playanim", "Sc2_Forad6 OnIdle") -- ARG_FORAD.scr:35
    do return ctx:exit("") end -- ARG_FORAD.scr:37
end

script.labels["OnVoice6"] = function(ctx)
    -- ARG_FORAD.scr:40
    ctx:command("playsound", "\\voices\\cinema\\TheArgument\\06.wav, DoNothing, 100, 24000, FALSE, 100") -- ARG_FORAD.scr:44
    do return ctx:exit("") end -- ARG_FORAD.scr:45
end

script.labels["OnSpeak20"] = function(ctx)
    -- ARG_FORAD.scr:48
    ctx:command("playanim", "Sc2_Forad26 OnIdle") -- ARG_FORAD.scr:51
    do return ctx:exit("") end -- ARG_FORAD.scr:53
end

script.labels["OnVoice20"] = function(ctx)
    -- ARG_FORAD.scr:56
    ctx:command("playsound", "\\voices\\cinema\\TheArgument\\20.wav, DoNothing, 100, 24000, FALSE, 100") -- ARG_FORAD.scr:60
    do return ctx:exit("") end -- ARG_FORAD.scr:61
end

script.labels["OnIdle"] = function(ctx)
    -- ARG_FORAD.scr:65
    ctx:command("loopanim", "Sit 0 DoNothing") -- ARG_FORAD.scr:68
    do return ctx:exit("") end -- ARG_FORAD.scr:69
end

script.labels["Init"] = function(ctx)
    -- ARG_FORAD.scr:72
    ctx:command("loopanim", "Sit 0 DoNothing") -- ARG_FORAD.scr:75
    do return ctx:exit("") end -- ARG_FORAD.scr:76
end

script.labels["Onclap"] = function(ctx)
    -- ARG_FORAD.scr:79
    ctx:command("getrandomint", "1, 7 g_ntemp") -- ARG_FORAD.scr:81
    if ctx:condition("g_ntemp==1") then -- ARG_FORAD.scr:83
        ctx:command("playsound", "\\sounds\\events\\clap01.wav, DoNothing, 100, 24000, FALSE, 100") -- ARG_FORAD.scr:84
        do return ctx:exit("") end -- ARG_FORAD.scr:85
    end -- ARG_FORAD.scr:86
    if ctx:condition("g_ntemp==2") then -- ARG_FORAD.scr:88
        ctx:command("playsound", "\\sounds\\events\\clap02.wav, DoNothing, 100, 24000, FALSE, 100") -- ARG_FORAD.scr:89
        do return ctx:exit("") end -- ARG_FORAD.scr:90
    end -- ARG_FORAD.scr:91
    if ctx:condition("g_ntemp==3") then -- ARG_FORAD.scr:93
        ctx:command("playsound", "\\sounds\\events\\clap03.wav, DoNothing, 100, 24000, FALSE, 100") -- ARG_FORAD.scr:94
        do return ctx:exit("") end -- ARG_FORAD.scr:95
    end -- ARG_FORAD.scr:96
    if ctx:condition("g_ntemp==4") then -- ARG_FORAD.scr:98
        ctx:command("playsound", "\\sounds\\events\\clap04.wav, DoNothing, 100, 24000, FALSE, 100") -- ARG_FORAD.scr:99
        do return ctx:exit("") end -- ARG_FORAD.scr:100
    end -- ARG_FORAD.scr:101
    if ctx:condition("g_ntemp==5") then -- ARG_FORAD.scr:103
        ctx:command("playsound", "\\sounds\\events\\clap05.wav, DoNothing, 100, 24000, FALSE, 100") -- ARG_FORAD.scr:104
        do return ctx:exit("") end -- ARG_FORAD.scr:105
    end -- ARG_FORAD.scr:106
    if ctx:condition("g_ntemp==6") then -- ARG_FORAD.scr:108
        ctx:command("playsound", "\\sounds\\events\\clap06.wav, DoNothing, 100, 24000, FALSE, 100") -- ARG_FORAD.scr:109
        do return ctx:exit("") end -- ARG_FORAD.scr:110
    end -- ARG_FORAD.scr:111
    if ctx:condition("g_ntemp==7") then -- ARG_FORAD.scr:113
        ctx:command("playsound", "\\sounds\\events\\clap07.wav, DoNothing, 100, 24000, FALSE, 100") -- ARG_FORAD.scr:114
...
script.labels["Main"] = function(ctx)
    -- ARG_FORAD.scr:121
    -- TraceOn ;delete me!!
    ctx:command("onpoststartworld", "Init") -- ARG_FORAD.scr:125
    ctx:command("onpostminisaveload", "Init") -- ARG_FORAD.scr:126
    ctx:command("onpostsaveload", "Init") -- ARG_FORAD.scr:127
    ctx:command("wait", "1 .1 Init") -- ARG_FORAD.scr:128
    ctx:addTrigger("Speak6", "OnSpeak6") -- ARG_FORAD.scr:129
    ctx:command("addmodelkey", "Voice6 OnVoice6") -- ARG_FORAD.scr:130
    ctx:addTrigger("Speak20", "OnSpeak20") -- ARG_FORAD.scr:131
    ctx:command("addmodelkey", "Voice20 OnVoice20") -- ARG_FORAD.scr:132
    ctx:command("addmodelkey", "Done OnDone") -- ARG_FORAD.scr:133
    ctx:addTrigger("Clap", "OnApplause") -- ARG_FORAD.scr:134
    ctx:command("addmodelkey", "Clap OnCLap") -- ARG_FORAD.scr:135

...
RegisterEvent(6, "Legacy event 6", function()
    if IsQBitSet(QBit(6)) then return end
    if not evt.CheckMonstersKilled(4, 1, 0, true) then return end
...
RegisterEvent(7, "Legacy event 7", function()
    ...
    if not HasItem(619) then return end -- Pirate Leader's Key
    SetQBit(QBit(214))
    return
end)
...
RegisterEvent(451, "Enter the Submarine", function()
    ...
    if HasItem(619) then -- Pirate Leader's Key
        evt.ShowMovie("\"Subcut\" ", false)
        evt.MoveToMap(-2416, 1850, -687, 1536, 0, 0, 0, 0)
        return
    end

...
#include "game/OutdoorPresentationController.h"
...
)
{
    ...
    m_baseTexturedTerrainVertices = texturedTerrainVertices;
    m_animatedTexturedTerrainVertices = texturedTerrainVertices;
    ...
    {
        m_texturedTerrainVertexBufferHandle = bgfx::createDynamicVertexBuffer(
            bgfx::copy(
    ...
    if (m_pOutdoorSceneRuntime != nullptr && m_pOutdoorSceneRuntime->localEventIrProgram())
    {
        queueEventSpellBillboardTextureWarmup(*m_pOutdoorSceneRuntime->localEventIrProgram());
    }

    if (m_pOutdoorSceneRuntime != nullptr && m_pOutdoorSceneRuntime->globalEventIrProgram())
    {
        queueEventSpellBillboardTextureWarmup(*m_pOutdoorSceneRuntime->globalEventIrProgram());
    }
...
void OutdoorGameView::render(int width, int height, float mouseWheelDelta, float deltaSeconds)
{
...

    {
        ...
        {
            if (!m_baseTexturedTerrainVertices.empty()
                && m_baseTexturedTerrainVertices.size() == m_animatedTexturedTerrainVertices.size())
            {
                m_animatedTexturedTerrainVertices = m_baseTexturedTerrainVertices;

                bgfx::update(
                    m_texturedTerrainVertexBufferHandle,
                    0,
                    bgfx::copy(
                        m_animatedTexturedTerrainVertices.data(),
                        static_cast<uint32_t>(m_animatedTexturedTerrainVertices.size() * sizeof(TexturedTerrainVertex))
                    )
                );
            }
...
const OutdoorGameView::SkyTextureHandle *OutdoorGameView::findSkyTexture(const std::string &textureName) const
{
    const std::string normalizedTextureName = toLowerCopy(textureName);
    const auto it = m_skyTextureIndexByName.find(normalizedTextureName);

    if (it == m_skyTextureIndexByName.end() || it->second >= m_skyTextureHandles.size())
    {
        return nullptr;
    }

    return &m_skyTextureHandles[it->second];
}

const OutdoorGameView::SkyTextureHandle *OutdoorGameView::ensureSkyTexture(const std::string &textureName)
{
    ...
    if (const SkyTextureHandle *pExistingTexture = findSkyTexture(textureName))
    {
        return pExistingTexture;
    }

    const std::optional<std::string> bitmapPath = findCachedAssetPath("Data/bitmaps", textureName + ".bmp");
    ...
    const std::optional<std::vector<uint8_t>> bitmapBytes = readCachedBinaryFile(*bitmapPath);
    ...
    m_skyTextureHandles.push_back(std::move(textureHandle));
    m_skyTextureIndexByName[m_skyTextureHandles.back().textureName] = m_skyTextureHandles.size() - 1;
    return &m_skyTextureHandles.back();
}

void OutdoorGameView::renderOutdoorSky(
    uint16_t viewId,
    uint16_t viewWidth,
    uint16_t viewHeight,
...
)
{
    ...
    const SkyTextureHandle *pTexture = ensureSkyTexture(pAtmosphereState->skyTextureName);
    ...
    const float skyUpdateElapsedTime = std::floor(m_elapsedTime * 30.0f) / 30.0f;
    ...
    {
        m_skyVertexBufferHandle = bgfx::createDynamicVertexBuffer(
            vertexCount,
            TexturedTerrainVertex::ms_layout,
            BGFX_BUFFER_NONE
        );
    ...
    if (m_cachedSkyVertices.size() != vertexCount)
    {
        m_cachedSkyVertices.resize(vertexCount);
    }

    if (m_lastSkyUpdateElapsedTime != skyUpdateElapsedTime || m_cachedSkyTextureName != pTexture->textureName)
    {
        ...
        {
            const float phi0 =
                (static_cast<float>(verticalIndex) / static_cast<float>(SkyDomeVerticalSegmentCount)) * (Pi * 0.5f);
            const float phi1 =
            ...
            for (uint32_t horizontalIndex = 0; horizontalIndex < SkyDomeHorizontalSegmentCount; ++horizontalIndex)
            {
                ...
                m_cachedSkyVertices[vertexIndex++] = {x00, y00, z0, u0, v0};
                m_cachedSkyVertices[vertexIndex++] = {x10, y10, z1, u0, v1};
                m_cachedSkyVertices[vertexIndex++] = {x11, y11, z1, u1, v1};
                m_cachedSkyVertices[vertexIndex++] = {x00, y00, z0, u0, v0};
                m_cachedSkyVertices[vertexIndex++] = {x11, y11, z1, u1, v1};
                m_cachedSkyVertices[vertexIndex++] = {x01, y01, z0, u1, v0};
            }
        }

        bgfx::update(
            m_skyVertexBufferHandle,
            0,
            bgfx::copy(
                m_cachedSkyVertices.data(),
                static_cast<uint32_t>(m_cachedSkyVertices.size() * sizeof(TexturedTerrainVertex))
            )
        );
        m_lastSkyUpdateElapsedTime = skyUpdateElapsedTime;
        m_cachedSkyTextureName = pTexture->textureName;
    }

    float modelMatrix[16] = {};
    bx::mtxIdentity(modelMatrix);
    bgfx::setTransform(modelMatrix);
    bgfx::setVertexBuffer(0, m_skyVertexBufferHandle, 0, vertexCount);
...
void OutdoorGameView::renderOutdoorDarknessOverlay(
    uint16_t viewId,
    const bx::Vec3 &cameraPosition,
    const bx::Vec3 &cameraForward,
    const bx::Vec3 &cameraRight,
...
{
    if (!bgfx::isValid(m_programHandle) || overlayAlpha <= 0.0f)
    {
        return;
    }
    ...
    const std::array<TerrainVertex, 6> vertices = {{
...
void OutdoorGameView::shutdown()
{
    ...
    if (bgfx::getInternalData() == nullptr)
    {
        ...
        for (SkyTextureHandle &textureHandle : m_skyTextureHandles)
        {
            textureHandle.textureHandle = BGFX_INVALID_HANDLE;
        }
        ...
        m_skyTextureHandles.clear();
        m_skyTextureIndexByName.clear();
...
GameplayDialogController::Context OutdoorGameView::createGameplayDialogContext(EventRuntimeState &eventRuntimeState)
{
    ...
    callbacks.executeNpcTopicEvent =
        [this](uint16_t eventId, size_t &previousMessageCount)
        {
            return m_pOutdoorSceneRuntime != nullptr
                && m_pOutdoorSceneRuntime->executeEventById(
                    std::nullopt,
                    eventId,
                    std::nullopt,
    ...
    return {
        ...
        m_pOutdoorSceneRuntime != nullptr ? &m_pOutdoorSceneRuntime->globalEventIrProgram() : nullptr,
...
void OutdoorGameView::executeActiveDialogAction()
{
    ...
    GameplayDialogController::Context context = createGameplayDialogContext(*pEventRuntimeState);
    ...
    {
        openPendingEventDialog(result.previousMessageCount, result.allowNpcFallbackContent);
    }
}
...
{
    GameplayDialogController::Context context = createGameplayDialogContext(eventRuntimeState);
    return m_gameplayDialogController.rejectClosedHouseInteraction(context, houseEntry);
}
...
void OutdoorGameView::updateCharacterInspectOverlayState(int width, int height)
{
    ...
    if (m_characterPage == CharacterPage::Stats)
    {
        ...
        {
            const HudLayoutElement *pLabelLayout = findHudLayoutElement(row.pLabelId);
            const HudLayoutElement *pValueLayout = findHudLayoutElement(row.pValueId);
            ...
            const std::optional<ResolvedHudLayoutElement> resolvedLabel = resolveHudLayoutElement(
                row.pLabelId,
                width,
                height,
                pLabelLayout->width,
                pLabelLayout->height);
            const std::optional<ResolvedHudLayoutElement> resolvedValue = resolveHudLayoutElement(
                row.pValueId,
                width,
                height,
                pValueLayout->width,
            ...
            if (!isPointerInsideResolvedElement(rowRect, mouseX, mouseY))
            {
                continue;
            }
    ...
    const auto tryShowSkillPopup =
            ...
            const std::vector<CharacterSkillUiRow> &rows) -> bool
        {
            ...
            const HudLayoutElement *pRegionLayout = findHudLayoutElement(pRegionId);
            const HudLayoutElement *pLevelLayout = findHudLayoutElement(pLevelHeaderId);
            ...
            const std::optional<ResolvedHudLayoutElement> resolvedRegion = resolveHudLayoutElement(
                pRegionId,
                width,
                height,
                pRegionLayout->width,
                pRegionLayout->height);
            const std::optional<ResolvedHudLayoutElement> resolvedLevelHeader = resolveHudLayoutElement(
                pLevelHeaderId,
                width,
                height,
                pLevelLayout->width,
...
void OutdoorGameView::updateBuffInspectOverlayState(int width, int height)
{
    ...
    for (const PartyBuffInspectTarget &target : BuffTargets)
    {
        ...
        const HudLayoutElement *pLayout = findHudLayoutElement(target.pLayoutId);
    ...
    {
        const HudLayoutElement *pPanelLayout = findHudLayoutElement(panelTarget.pLayoutId);
        ...
        const std::optional<ResolvedHudLayoutElement> resolvedPanel = resolveHudLayoutElement(
        ...
        if (!resolvedPanel || !isPointerInsideResolvedElement(*resolvedPanel, mouseX, mouseY))
        {
            continue;
        }
...
void OutdoorGameView::updateCharacterDetailOverlayState(int width, int height)
{
    ...
    if (m_characterScreenOpen)
    {
        ...
        {
            sourceRect = resolveHudLayoutElement(
                "CharacterDollPanel",
                width,
                height,
                pDollLayout->width,
    ...
    if (pCharacter == nullptr || !sourceRect || !isPointerInsideResolvedElement(*sourceRect, mouseX, mouseY))
    {
        return;
    }
...
void OutdoorGameView::returnToHouseBankMainDialog()
{
    ...
    GameplayDialogController::Context context = createGameplayDialogContext(*pEventRuntimeState);
    const GameplayDialogController::Result result = m_gameplayDialogController.returnToHouseBankMainDialog(context);
...
void OutdoorGameView::openDebugNpcDialogue(uint32_t npcId)
{
    ...
    GameplayDialogController::Context context = createGameplayDialogContext(*pEventRuntimeState);
    const GameplayDialogController::Result result = m_gameplayDialogController.openNpcDialogue(context, npcId);
...
{
    if (currentHudScreenState() != HudScreenState::Character
        || m_pOutdoorPartyRuntime == nullptr
        || !bgfx::isValid(m_texturedTerrainProgramHandle)
        || !bgfx::isValid(m_terrainTextureSamplerHandle)
        || width <= 0
    ...
    const auto hasVisibleCharacterAncestors =
        [this](const HudLayoutElement &layout) -> bool
        {
            const std::string activePageRootId =
                m_characterPage == CharacterPage::Stats
                ? "characterstatspage"
                : (m_characterPage == CharacterPage::Skills
                    ? "characterskillspage"
                    : (m_characterPage == CharacterPage::Inventory
    ...
    const std::vector<std::string> orderedCharacterLayoutIds = sortedHudLayoutIdsForScreen("Character");
    ...
    const float skillRowHeight = pSkillRowFont != nullptr
        ? static_cast<float>(std::max(1, pSkillRowFont->fontHeight - 3))
        : 11.0f;
    const CharacterDollEntry *pCharacterDollEntry = resolveCharacterDollEntry(m_pCharacterDollTable, pCharacter);
    const CharacterDollTypeEntry *pCharacterDollType =
        pCharacterDollEntry != nullptr && m_pCharacterDollTable != nullptr
        ? m_pCharacterDollTable->getDollType(pCharacterDollEntry->dollTypeId)
        : nullptr;
    ...
    setCharacterSkillRegionHeight(m_hudLayoutRuntimeHeightOverrides, skillRowHeight, "CharacterSkillsWeaponsListRegion", skillUiData.weaponRows.size());
    setCharacterSkillRegionHeight(m_hudLayoutRuntimeHeightOverrides, skillRowHeight, "CharacterSkillsMagicListRegion", skillUiData.magicRows.size());
    setCharacterSkillRegionHeight(m_hudLayoutRuntimeHeightOverrides, skillRowHeight, "CharacterSkillsArmorListRegion", skillUiData.armorRows.size());
    ...
    for (const std::string &layoutId : orderedCharacterLayoutIds)
    {
        ...
        if (normalizedLayoutId == "characterdolljewelryoverlaypanel" && !m_characterDollJewelryOverlayOpen)
        {
            continue;
        }
        ...
        if (normalizedLayoutId == "characterdollbackground")
        {
                ...
                && pCharacterDollEntry->backgroundAsset != "none")
            {
                const HudTextureHandle *pBackgroundTexture =
                    const_cast<OutdoorGameView *>(this)->ensureHudTextureLoaded(pCharacterDollEntry->backgroundAsset);
            ...
            if (!assetName.empty())
            {
                const HudTextureHandle *pTexture =
                    const_cast<OutdoorGameView *>(this)->ensureHudTextureLoaded(assetName);
        ...
        if (toLowerCopy(layoutId) == "characterpaperdollregion"
            && pCharacter != nullptr
            && !pCharacter->portraitTextureName.empty())
        {
            ...
            if (pPortraitTexture != nullptr && pPortraitTexture->width > 0 && pPortraitTexture->height > 0)
            {
                ...
                submitTexturedQuad(*pPortraitTexture, portraitX, portraitY, portraitWidth, portraitHeight);
            }
        }

        const std::string *pAssetName =
            resolveInteractiveAssetName(*pLayout, *resolved, characterMouseX, characterMouseY, isLeftMousePressed);
        ...
        if (slot && isVisibleInCharacterDollOverlay(*slot, m_characterDollJewelryOverlayOpen))
        {
    ...
    for (const std::string &layoutId : orderedCharacterLayoutIds)
    {
        ...
        HudLayoutElement layoutForRender = *pLayout;
        ...
        label = replaceAll(
            ...
            pCharacter != nullptr
                ? (!pCharacter->className.empty() ? pCharacter->className : pCharacter->role)
                : "");
        label = replaceAll(
            label,
            "{quick_stats}",
            pCharacter != nullptr
                ? ("HP " + std::to_string(pCharacter->health) + "/" + std::to_string(pCharacter->maxHealth)
                    ...
                    + std::to_string(pCharacter->maxSpellPoints))
                : "");
        ...
        renderLayoutLabel(layoutForRender, *resolved, label);
    }

    const auto renderSkillGroup =
        [this, width, height, &hasVisibleCharacterAncestors, &shouldRenderInCurrentPass, skillRowHeight](
            const char *pRegionId,
            const char *pLevelHeaderId,
            const std::vector<CharacterSkillUiRow> &rows)
        {
            ...
            const float rowHeightPixels = skillRowHeight * resolvedRegion->scale;
            const float nameWidth = std::max(0.0f, resolvedLevelHeader->x - resolvedRegion->x - 6.0f * resolvedRegion->scale);
            const std::vector<CharacterSkillUiRow> displayRows = rows.empty()
                ? std::vector<CharacterSkillUiRow>{{"", "None", "", false}}
                : rows;
            ...
            for (size_t rowIndex = 0; rowIndex < displayRows.size(); ++rowIndex)
            {
                ...
                HudLayoutElement nameLayout = {};
                nameLayout.fontName = "Lucida";
                nameLayout.textColorAbgr = textColorAbgr;
                nameLayout.textAlignX = HudTextAlignX::Left;
                nameLayout.textAlignY = HudTextAlignY::Middle;
                ResolvedHudLayoutElement nameResolved = {};
                ...
                renderLayoutLabel(nameLayout, nameResolved, row.label);

                if (!row.level.empty())
                {
                    HudLayoutElement levelLayout = {};
                    levelLayout.fontName = "Lucida";
                    levelLayout.textColorAbgr = textColorAbgr;
                    levelLayout.textAlignX = HudTextAlignX::Right;
                    levelLayout.textAlignY = HudTextAlignY::Middle;
                    ResolvedHudLayoutElement levelResolved = {};
                    ...
                    renderLayoutLabel(levelLayout, levelResolved, row.level);
                }
            }
        };
...
void OutdoorGameView::renderItemInspectOverlay(int width, int height) const
{
    ...
    const std::string itemDescription = showBrokenOnly
        ...
        : (showUnidentifiedOnly
            ? "Not identified"
            : pItemDefinition->notes);
    const int resolvedItemValue =
        m_itemInspectOverlay.hasValueOverride
        ? m_itemInspectOverlay.valueOverride
    ...
    const HudFontHandle *pDetailFont = findHudFont(pDetailValueLayout->fontName);
    ...
    const float detailTextWidthScaled =
        std::max(0.0f, pDetailValueLayout->width * popupScale - std::abs(pDetailValueLayout->textPadX * popupScale) * 2.0f);
    const float detailTextWidth =
        std::max(0.0f, detailTextWidthScaled / std::max(0.1f, detailFontScale));
    const std::vector<std::string> specialDetailLines =
        showSpecialDetail && pDetailFont != nullptr
        ? wrapHudTextToWidth(*pDetailFont, itemSpecialDetail, detailTextWidth)
    ...
    {
        ResolvedHudLayoutElement specialRowRect = *detailRowBaseRect;
        ...
        const ResolvedHudLayoutElement specialValueRect = resolveAttachedHudLayoutRect(
            pDetailValueLayout->attachTo,
            specialRowRect,
            pDetailValueLayout->width * popupScale,
        ...
        if (pDetailFont != nullptr && !specialDetailLines.empty())
        {
            bgfx::TextureHandle coloredMainTextureHandle =
                ensureHudFontMainTextureColor(*pDetailFont, pDetailValueLayout->textColorAbgr);
            ...
            {
                renderHudFontLayer(*pDetailFont, pDetailFont->shadowTextureHandle, wrappedLine, textX, textY, detailFontScale);
                renderHudFontLayer(*pDetailFont, coloredMainTextureHandle, wrappedLine, textX, textY, detailFontScale);
                textY += detailLineHeight;
            }
        }
        else
        {
            renderLayoutLabel(*pDetailValueLayout, specialValueRect, itemSpecialDetail);
        }
    }
...
{
    if (!m_characterInspectOverlay.active
        || !bgfx::isValid(m_texturedTerrainProgramHandle)
        || !bgfx::isValid(m_terrainTextureSamplerHandle)
        || width <= 0
        || height <= 0)
    ...
    const HudLayoutElement *pRootLayout = findHudLayoutElement("CharacterInspectRoot");
    const HudLayoutElement *pTitleLayout = findHudLayoutElement("CharacterInspectTitle");
    const HudLayoutElement *pBodyLayout = findHudLayoutElement("CharacterInspectBody");
    const HudLayoutElement *pExpertLayout = findHudLayoutElement("CharacterInspectExpert");
    const HudLayoutElement *pMasterLayout = findHudLayoutElement("CharacterInspectMaster");
    const HudLayoutElement *pGrandmasterLayout = findHudLayoutElement("CharacterInspectGrandmaster");
    ...
    const std::vector<std::string> bodyLines =
        pBodyFont != nullptr
        ? wrapHudTextToWidth(*pBodyFont, m_characterInspectOverlay.body, bodyWidth)
        : std::vector<std::string>{m_characterInspectOverlay.body};
    const float bodyHeight = bodyLines.empty() ? bodyLineHeight : bodyLineHeight * static_cast<float>(bodyLines.size());
    const bool showExpert = m_characterInspectOverlay.expert.visible && !m_characterInspectOverlay.expert.text.empty();
    const bool showMaster = m_characterInspectOverlay.master.visible && !m_characterInspectOverlay.master.text.empty();
    const bool showGrandmaster =
        m_characterInspectOverlay.grandmaster.visible && !m_characterInspectOverlay.grandmaster.text.empty();
    ...
    const std::vector<std::string> expertLines =
        showExpert ? resolveWrappedMasteryLines(*pExpertLayout, m_characterInspectOverlay.expert.text) : std::vector<std::string>{};
    const std::vector<std::string> masterLines =
        showMaster ? resolveWrappedMasteryLines(*pMasterLayout, m_characterInspectOverlay.master.text) : std::vector<std::string>{};
    const std::vector<std::string> grandmasterLines =
        showGrandmaster
            ? resolveWrappedMasteryLines(*pGrandmasterLayout, m_characterInspectOverlay.grandmaster.text)
    ...
    const float popupGap = 12.0f * popupScale;
    float rootX = m_characterInspectOverlay.sourceX + m_characterInspectOverlay.sourceWidth + popupGap;

    if (rootX + rootWidth > uiViewport.x + uiViewport.width)
    {
        rootX = m_characterInspectOverlay.sourceX - rootWidth - popupGap;
    }

    rootX = std::clamp(rootX, uiViewport.x, uiViewport.x + uiViewport.width - rootWidth);
    float rootY = m_characterInspectOverlay.sourceY + (m_characterInspectOverlay.sourceHeight - rootHeight) * 0.5f;
    rootY = std::clamp(rootY, uiViewport.y, uiViewport.y + uiViewport.height - rootHeight);
    ...
    const std::vector<std::string> orderedLayoutIds = sortedHudLayoutIdsForScreen("CharacterInspect");
    ...
    renderSingleLine("CharacterInspectTitle", m_characterInspectOverlay.title);

    const std::optional<ResolvedHudLayoutElement> bodyBaseRect = resolveLayout("CharacterInspectBody");
    ...
    if (bodyBaseRect && pBodyFont != nullptr)
    {
        ...
        const auto renderMasteryRow =
            [this, popupScale, &masteryColorForAvailability](
                const HudLayoutElement &baseLayout,
                const ResolvedHudLayoutElement &baseResolved,
                float rowY,
                const CharacterInspectMasteryLine &line,
                const std::vector<std::string> &wrappedLines) -> float
            {
                ...
                const HudFontHandle *pFont = findHudFont(baseLayout.fontName);
                ...
                bgfx::TextureHandle coloredMainTextureHandle =
                    ensureHudFontMainTextureColor(*pFont, masteryColorForAvailability(line.availability));
        ...
        const std::optional<ResolvedHudLayoutElement> expertRect = resolveLayout("CharacterInspectExpert");
        const std::optional<ResolvedHudLayoutElement> masterRect = resolveLayout("CharacterInspectMaster");
        const std::optional<ResolvedHudLayoutElement> grandmasterRect = resolveLayout("CharacterInspectGrandmaster");

        if (expertRect)
        {
            nextRowY = renderMasteryRow(*pExpertLayout, *expertRect, nextRowY, m_characterInspectOverlay.expert, expertLines);
        ...
        if (masterRect)
        {
            nextRowY = renderMasteryRow(*pMasterLayout, *masterRect, nextRowY, m_characterInspectOverlay.master, masterLines);
...
{
    if (!m_buffInspectOverlay.active
        || !bgfx::isValid(m_texturedTerrainProgramHandle)
        || !bgfx::isValid(m_terrainTextureSamplerHandle)
        || width <= 0
        || height <= 0)
    ...
    const HudLayoutElement *pRootLayout = findHudLayoutElement("BuffInspectRoot");
    const HudLayoutElement *pTitleLayout = findHudLayoutElement("BuffInspectTitle");
    const HudLayoutElement *pBodyLayout = findHudLayoutElement("BuffInspectBody");
    ...
    const std::vector<std::string> bodyLines =
        pBodyFont != nullptr
            ? wrapHudTextToWidth(*pBodyFont, m_buffInspectOverlay.body, bodyWidth)
            : std::vector<std::string>{m_buffInspectOverlay.body};
    const float bodyHeight = bodyLines.empty() ? bodyLineHeight : bodyLineHeight * static_cast<float>(bodyLines.size());
    ...
    float rootX = m_buffInspectOverlay.sourceX + m_buffInspectOverlay.sourceWidth + popupGap;

    if (rootX + rootWidth > uiViewport.x + uiViewport.width)
    {
        rootX = m_buffInspectOverlay.sourceX - rootWidth - popupGap;
    }

    rootX = std::clamp(rootX, uiViewport.x, uiViewport.x + uiViewport.width - rootWidth);
    float rootY = m_buffInspectOverlay.sourceY + (m_buffInspectOverlay.sourceHeight - rootHeight) * 0.5f;
    rootY = std::clamp(rootY, uiViewport.y, uiViewport.y + uiViewport.height - rootHeight);
    const ResolvedHudLayoutElement rootRect = {
        std::round(rootX),
        std::round(rootY),
    ...
    const std::vector<std::string> orderedLayoutIds = sortedHudLayoutIdsForScreen("BuffInspect");
    ...
    const std::optional<ResolvedHudLayoutElement> titleRect = resolveLayout("BuffInspectTitle");

    if (titleRect)
    {
        renderLayoutLabel(*pTitleLayout, *titleRect, m_buffInspectOverlay.title);
    }
    ...
    const std::optional<ResolvedHudLayoutElement> bodyBaseRect = resolveLayout("BuffInspectBody");
...
{
    if (!m_characterDetailOverlay.active
        || !bgfx::isValid(m_texturedTerrainProgramHandle)
        || !bgfx::isValid(m_terrainTextureSamplerHandle)
        || width <= 0
        || height <= 0)
    ...
    const HudLayoutElement *pRootLayout = findHudLayoutElement("CharacterDetailRoot");
    const HudLayoutElement *pTitleLayout = findHudLayoutElement("CharacterDetailTitle");
    const HudLayoutElement *pBodyLayout = findHudLayoutElement("CharacterDetailBody");
    ...
    const std::vector<std::string> bodyLines =
        pBodyFont != nullptr
            ? wrapHudTextToWidth(*pBodyFont, m_characterDetailOverlay.body, bodyWidth)
            : std::vector<std::string>{m_characterDetailOverlay.body};
    const float bodyHeight = bodyLines.empty() ? bodyLineHeight : bodyLineHeight * static_cast<float>(bodyLines.size());
    ...
    const float popupGap = 12.0f * popupScale;
    float rootX = m_characterDetailOverlay.sourceX + m_characterDetailOverlay.sourceWidth + popupGap;

    if (rootX + rootWidth > uiViewport.x + uiViewport.width)
    {
        rootX = m_characterDetailOverlay.sourceX - rootWidth - popupGap;
    }

    rootX = std::clamp(rootX, uiViewport.x, uiViewport.x + uiViewport.width - rootWidth);
    float rootY = m_characterDetailOverlay.sourceY + (m_characterDetailOverlay.sourceHeight - rootHeight) * 0.5f;
    rootY = std::clamp(rootY, uiViewport.y, uiViewport.y + uiViewport.height - rootHeight);
    const ResolvedHudLayoutElement rootRect = {
        std::round(rootX),
        std::round(rootY),
    ...
    const std::vector<std::string> orderedLayoutIds = sortedHudLayoutIdsForScreen("CharacterDetail");
    ...
    const std::optional<ResolvedHudLayoutElement> titleRect = resolveLayout("CharacterDetailTitle");

    if (titleRect)
    {
        renderLayoutLabel(*pTitleLayout, *titleRect, m_characterDetailOverlay.title);
    }
    ...
    const std::optional<ResolvedHudLayoutElement> bodyBaseRect = resolveLayout("CharacterDetailBody");
...
{
    if (!m_actorInspectOverlay.active
        || m_pOutdoorWorldRuntime == nullptr
        || !m_monsterTable.has_value()
        || !bgfx::isValid(m_programHandle)
        || !bgfx::isValid(m_texturedTerrainProgramHandle)
        || !bgfx::isValid(m_terrainTextureSamplerHandle)
        || width <= 0
    ...
    const OutdoorWorldRuntime::MapActorState *pActorState =
        m_pOutdoorWorldRuntime->mapActorState(m_actorInspectOverlay.runtimeActorIndex);
    ...
    const MonsterTable::MonsterStatsEntry *pStats = m_monsterTable->findStatsById(pActorState->monsterId);
    ...
    const HudLayoutElement *pRootLayout = findHudLayoutElement("ActorInspectRoot");
    const HudLayoutElement *pPreviewLayout = findHudLayoutElement("ActorInspectPreviewFrame");
    const HudLayoutElement *pEffectsBodyLayout = findHudLayoutElement("ActorInspectEffectsBody");
    const HudLayoutElement *pHealthBarBackgroundLayout = findHudLayoutElement("ActorInspectHealthBarBackground");
    const HudLayoutElement *pHealthBarFillLayout = findHudLayoutElement("ActorInspectHealthBarFill");

    if (pRootLayout == nullptr
        || pPreviewLayout == nullptr
        || pEffectsBodyLayout == nullptr
    ...
    const float popupGap = 12.0f * popupScale;
    float rootX = m_actorInspectOverlay.sourceX + m_actorInspectOverlay.sourceWidth + popupGap;

    if (rootX + rootWidth > uiViewport.x + uiViewport.width)
    {
        rootX = m_actorInspectOverlay.sourceX - rootWidth - popupGap;
    }

    rootX = std::clamp(rootX, uiViewport.x, uiViewport.x + uiViewport.width - rootWidth);
    float rootY = m_actorInspectOverlay.sourceY + (m_actorInspectOverlay.sourceHeight - rootHeight) * 0.5f;
    rootY = std::clamp(rootY, uiViewport.y, uiViewport.y + uiViewport.height - rootHeight);
    ...
    const auto submitActorPreviewQuad =
        [this](
    ...
    const auto submitSolidQuad =
        [this, &submitTexturedQuad](float x, float y, float quadWidth, float quadHeight, uint32_t abgr)
        {
            if (quadWidth <= 0.0f || quadHeight <= 0.0f)
            {
                return;
            ...
            const HudTextureHandle *pSolidTexture =
                const_cast<OutdoorGameView *>(this)->ensureSolidHudTextureLoaded(textureName, solidTextureAbgr);
            ...
            submitTexturedQuad(*pSolidTexture, x, y, quadWidth, quadHeight);
        };
    ...
    const std::optional<ResolvedHudLayoutElement> previewRect = resolveLayout("ActorInspectPreviewFrame");
    ...
    const std::vector<std::string> orderedLayoutIds = sortedHudLayoutIdsForScreen("ActorInspect");
    ...
    const float healthRatio =
        pActorState->maxHp > 0
            ? std::clamp(static_cast<float>(pActorState->currentHp) / static_cast<float>(pActorState->maxHp), 0.0f, 1.0f)
            : 0.0f;
    ...
    if (const std::optional<ResolvedHudLayoutElement> healthBackgroundRect = resolveLayout("ActorInspectHealthBarBackground"))
    {
        submitSolidQuad(
    ...
    if (const std::optional<ResolvedHudLayoutElement> healthFillRect = resolveLayout("ActorInspectHealthBarFill"))
    {
        submitSolidQuad(
            healthFillRect->x,
            healthFillRect->y,
    ...
    if (m_outdoorActorPreviewBillboardSet)
    {
        ...
        const SpriteFrameEntry *pFrame =
            m_outdoorActorPreviewBillboardSet->spriteFrameTable.getFrame(spriteFrameIndex, currentAnimationTicks());

        if (pFrame == nullptr)
        {
            pFrame = m_outdoorActorPreviewBillboardSet->spriteFrameTable.getFrame(spriteFrameIndex, 0);
        }
        ...
        if (pFrame != nullptr)
        {
            ...
            if (pTexture != nullptr && bgfx::isValid(pTexture->textureHandle))
            {
                if (previewRect)
                {
                    ...
                    const uint16_t scissorX =
                        static_cast<uint16_t>(std::clamp(std::lround(previewRect->x + previewInnerInset), 0l, long(width)));
                    const uint16_t scissorY =
                        static_cast<uint16_t>(std::clamp(std::lround(previewRect->y + previewInnerInset), 0l, long(height)));
    ...
    {
        const HudLayoutElement *pLayout = findHudLayoutElement(pLabelId);
...
{
    return m_uiLayoutManager.maxZIndexForScreen(screen);
}
...
const std::string *OutdoorGameView::resolveInteractiveAssetName(
    const HudLayoutElement &layout,
    const ResolvedHudLayoutElement &resolved,
    float pointerX,
    float pointerY,
    ...
    std::unordered_set<std::string> &visited) const
{
    ...
    const HudLayoutElement &element = *pElement;
...
{
    OutdoorBillboardRenderer::renderDecorationBillboards(*this, viewId, viewWidth, viewHeight, pViewMatrix, cameraPosition);
}
...
{
    OutdoorBillboardRenderer::renderActorPreviewBillboards(*this, viewId, pViewMatrix, cameraPosition);
}
...
void OutdoorGameView::renderRuntimeWorldItems(
    ...
    const bx::Vec3 &cameraPosition)
{
    OutdoorBillboardRenderer::renderRuntimeWorldItems(*this, viewId, pViewMatrix, cameraPosition);
}
...
void OutdoorGameView::renderRuntimeProjectiles(
    ...
    const bx::Vec3 &cameraPosition)
{
    OutdoorBillboardRenderer::renderRuntimeProjectiles(*this, viewId, pViewMatrix, cameraPosition);
}
...
{
    OutdoorBillboardRenderer::renderSpriteObjectBillboards(*this, viewId, pViewMatrix, cameraPosition);
}
...
bool OutdoorGameView::tryActivateInspectEvent(const InspectHit &inspectHit)
{
    ...
    if (inspectHit.kind == "actor" && inspectHit.npcId > 0)
    {
        ...
        const GameplayDialogController::Result result =
            m_gameplayDialogController.openNpcDialogue(context, static_cast<uint32_t>(inspectHit.npcId));
    ...
    if (inspectHit.kind == "actor" && m_npcDialogTable)
    {
        ...
        if (resolution)
        {
            ...
            faceTalkingActor();
            GameplayDialogController::Context context = createGameplayDialogContext(*pEventRuntimeState);
            const GameplayDialogController::Result result = m_gameplayDialogController.openNpcNews(
                context,
                resolution->npcId,
                resolution->newsId,
                inspectHit.name,
    ...
    const bool executed = m_pOutdoorSceneRuntime != nullptr
        && m_pOutdoorSceneRuntime->executeEventById(
            m_pOutdoorSceneRuntime->localEventIrProgram(),
            eventId,
            decorationContext,
...
void OutdoorGameView::updateCameraFromInput(float deltaSeconds)
{
            ...
            [this, requireGameplayReady, hasActiveLootView, hasPendingSpellCast](const std::optional<size_t> &memberIndex)
            {
                if (memberIndex)
                {
                    ...
                    if (trySelectPartyMember(*memberIndex, requireGameplayReady))
                    {
                        ...
                        {
                            openInventoryNestedOverlay(InventoryNestedOverlayMode::ChestTransfer);
                        }
    ...
    if (m_spellbook.active)
    {
        GameplayPartyOverlayInputController::handleSpellbookOverlayInput(*this, pKeyboardState, screenWidth, screenHeight);
        return;
    }

    if (m_characterScreenOpen)
    {
        GameplayPartyOverlayInputController::handleCharacterOverlayInput(*this, pKeyboardState, screenWidth, screenHeight);
        return;
    }
    ...
    const bool blockCameraRotation =
        m_buffInspectOverlay.active
        || m_characterDetailOverlay.active;
            ...
            if (!hasActiveLootView && !hasPendingSpellCast)
            {
                ...
                if (m_pOutdoorSceneRuntime != nullptr)
                {
                    const OutdoorSceneRuntime::AdvanceFrameResult frameAdvanceResult =
                        m_pOutdoorSceneRuntime->advanceFrame(movementInput, deltaSeconds);

                    applyPendingCombatEvents();

                    if (frameAdvanceResult.shouldOpenEventDialog)
                    {
                        openPendingEventDialog(frameAdvanceResult.previousMessageCount, true);
                    }
                }
            }
    ...
    if (pKeyboardState[SDL_SCANCODE_F1])
    {
        if (!m_triggerMeteorLatch)
        {
            const bool isDwiOutdoor =
                ...
                && m_outdoorMapData.has_value()
                && m_pOutdoorSceneRuntime != nullptr
                && m_pOutdoorSceneRuntime->localEventIrProgram().has_value();

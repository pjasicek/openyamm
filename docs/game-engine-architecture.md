...
- outdoor water animation is currently a temporary CPU-side terrain-atlas update in
  [`game/outdoor/OutdoorRenderer.cpp`](/home/pjasicek/github/OpenYAMM/game/outdoor/OutdoorRenderer.cpp)
  so DWI water and shoreline water can move now
- revisit this with a shader-side water path or water-specific terrain material so
  water animation stops depending on atlas subimage uploads and CPU recompositing

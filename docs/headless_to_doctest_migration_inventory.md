...
- Original headless cases inventoried: 279
- Moved to doctest after this inventory: 139
- Remaining classified headless cases: 140
- Remaining `doctest-direct`: 0
- `doctest-with-adaptation`: 0
- `stay-headless`: 140

The `doctest-direct` and `doctest-with-adaptation` sections below preserve the original migration backlog snapshot for
this loop. Current completion status and validation evidence live in `PROGRESS.md`; the `stay-headless` section below
is kept current as the authoritative remaining integration-level set.

Reclassification note:

- 2026-04-24: `generated_lua_event_scripts_load_for_every_scripted_map`,
  ...
  `lua_event_runtime_executes_scripted_map_handlers_without_errors`, and
  `outdoor_scene_yml_matches_legacy_ddm_authored_state` were moved back out of doctest and into the headless dialogue
  suite. They are valid integration coverage, but too expensive for the fast `openyamm_unit_tests` binary.
...
- `ordinary_damage_queues_minor_speech_reaction` was moved into `tests/AudioRegressionTests.cpp`.
- `major_damage_threshold_queues_minor_and_major_speech_reactions` was moved into
  `tests/AudioRegressionTests.cpp`.
- `damage_minor_speech_ignores_combat_cooldown` was moved into `tests/AudioRegressionTests.cpp`.
- `damage_major_speech_bypasses_active_speech_cooldowns` was moved into `tests/AudioRegressionTests.cpp`.
- `damage_impact_sound_request_uses_armor_family_mapping` was moved into `tests/AudioRegressionTests.cpp`.
...
- 007a `indoor` L5082: `indoor_movement_controller_crosses_portals_and_respects_mechanism_blocking`
...
- 009a `indoor` L5326: `indoor_d18_large_actor_crosses_portal_off_center`
...
- 011a `indoor` L5475: `indoor_d18_large_actor_crosses_face_1366`
- 012 `indoor` L5501: `party_spell_system_supports_indoor_shared_runtime_direct_actor_damage`
- 012a `indoor` L5582: `indoor_d18_naga_lintel_slide_preserves_tangential_progress`

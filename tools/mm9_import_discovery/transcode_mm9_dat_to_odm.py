...
def build_texture_size_index(extracted_root: Path | None) -> dict[str, tuple[int, int, Path]]:
    if extracted_root is None:
        return {}
    texture_root = extracted_root / "TEXTURES" / "TEXTURES"
...
def decode_dtx_pixels(path: Path, alias: str) -> tuple[int, int, bytearray, str]:
    ...
    if bpp == 3 and len(payload) >= width * height * 4:
        pixels = bytearray()
...
def build_aliases(texture_names: list[str]) -> dict[str, str]:
    ...
    for texture_name in texture_names:
        base = alias_base(texture_name)
        if len(base) <= BMODEL_TEXTURE_NAME_SIZE and base not in used:
            alias = base
        else:
            prefix = base[:7]
            alias = ""
            for index in range(1000):
                candidate = f"{prefix}{index:03d}"[:BMODEL_TEXTURE_NAME_SIZE]
...
def classify_face_attributes(model_name: str, texture_name: str, surface_flags: int) -> int:
    ...
    if normalized_model.startswith("aitrk"):
        return FACE_ATTRIBUTE_INVISIBLE | FACE_ATTRIBUTE_UNTOUCHABLE

    if (surface_flags & LT_SURFACE_FLAG_INVISIBLE) != 0:
        return FACE_ATTRIBUTE_INVISIBLE | FACE_ATTRIBUTE_UNTOUCHABLE

    if normalized_model == "ocean" or normalized_texture.startswith("sprites/water/"):
        return FACE_ATTRIBUTE_INVISIBLE | FACE_ATTRIBUTE_UNTOUCHABLE

    if normalized_model.startswith("bluewater") or texture_stem.startswith("water"):
        return FACE_ATTRIBUTE_FLUID | FACE_ATTRIBUTE_ANIMATED | FACE_ATTRIBUTE_UNTOUCHABLE

    if texture_stem.startswith("invisib") or texture_stem == "invisible" or texture_stem == "soundonly":
        return FACE_ATTRIBUTE_INVISIBLE | FACE_ATTRIBUTE_UNTOUCHABLE
...
def transcode_geometry(
    ...
    scale: float,
    texture_sizes: dict[str, tuple[int, int, Path]],
) -> tuple[list[OdmBModel], dict[str, dict[str, Any]], dict[str, Any]]:
    unique_textures = sorted({texture for model in dat_world.world_models for texture in model.textures})
    ...
    for model_index, model in enumerate(dat_world.world_models):
        ...
        for poly_index, poly in enumerate(model.polies):
            ...
            if len(source_indices) == 3:
                triangles.append((source_indices, source_uvs))
            else:
            ...
            for triangle_indices, triangle_uvs in triangles:
                ...
                # but reverse each emitted triangle so ODM plane normals point to the same side as the source DAT.
                triangle_indices = [triangle_indices[0], triangle_indices[2], triangle_indices[1]]
                triangle_uvs = [triangle_uvs[0], triangle_uvs[2], triangle_uvs[1]]
                if poly.plane_index < len(model.planes):
                    source_normal = transformed_lt_plane_normal(model.planes[poly.plane_index])
                    emitted_normal = compute_unit_normal(bmodel.vertices, triangle_indices)
                    if (
                ...
                face = OdmFace(
                    ...
                    texture_us=[uv[0] for uv in triangle_uvs],
                    texture_vs=[uv[1] for uv in triangle_uvs],
                    texture_alias=texture_alias,
                    bitmap_index=list(alias_metadata.keys()).index(texture_alias),
                    polygon_type=classify_polygon_type(bmodel.vertices, triangle_indices),
                    attributes=classify_face_attributes(bmodel.name, texture_name, surface.flags),
                    plane_normal=plane_normal,
                    plane_distance=clamped_plane_distance,
                )
                bmodel.faces.append(face)
                bmodel.source_poly_for_face.append(poly_index)
                bmodel.source_surface_for_face.append(poly.surface_index)
                bmodel.source_surface_flags_for_face.append(surface.flags)
                bmodel.source_texture_index_for_face.append(surface.texture_index)
    ...
    return bmodels, alias_metadata, stats
...
def build_odm_bytes(name: str, bmodels: list[OdmBModel]) -> bytes:
    ...
    write_fixed_string_at(data, 0x60, 0x20, "plansky1")
...
def main() -> int:
    ...
    model_instance_lines, model_assets = build_model_instance_lines(dat_world, args.scale)
    stats["model_instances"] = sum(int(asset["instance_count"]) for asset in model_assets)
    stats["unique_model_assets"] = len(model_assets)
    ...
    bitmap_directory_name = f"{output_name}.bitmaps"
    bitmap_dir = args.output_dir / bitmap_directory_name

    odm_path.write_bytes(build_odm_bytes(output_name, bmodels))
    bitmap_modes = write_alias_bitmaps(bitmap_dir, alias_metadata)
    write_scene_yml(scene_path, odm_path.name, source_metadata_path.name, model_instance_lines)

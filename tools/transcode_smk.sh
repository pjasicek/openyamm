...
usage()
{
    cat <<'EOF'
Usage:
  transcode_smk.sh -i INPUT_DIR -o OUTPUT_DIR

Description:
  Transcodes all .smk files in INPUT_DIR to .ogv files in OUTPUT_DIR.
  The script probes video/audio end times and trims output to the shortest
  valid stream end so looping behaves better for short house videos.
EOF
}
...
done < <(find "$input_dir" -maxdepth 1 -type f \( -iname '*.smk' \) -print0 | sort -z)

if [[ "$processed_count" -eq 0 ]]
then
    echo "No .smk files found in: $input_dir" >&2
    exit 1
fi

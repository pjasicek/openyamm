...
- For MM9 DAT/world import work, read `docs/mm9/MM9_DAT_FORMAT_NOTES.md` first. MM9 uses a locally verified DAT v66
  variant that is close to public LithTech 2.x/NOLF v66 references but not byte-identical.
- For MM9 dialogue/runtime integration, read `docs/mm9/MM9_DIALOGUE_RUNTIME_PIPELINE_CHECKLIST.md` first. Generated
  RUDE YAML, Lua scripts, state registries, and object bindings must be regenerated by the pipeline and covered by
  idempotency/losslessness tests rather than hand-edited.
...
- Treat QBits as global party/save state. Worlds and mods may own declared QBit ranges, and those bits merge into the
  shared registry and must not silently collide. MM9 is core content: MM9 script `Key` ids are qbit-backed through the
  reserved range `9000 + raw_mm9_key` while still being referenced semantically as `mm9.keys.<raw_id>` in generated
  data and scripts. Because this occupies ids above `10000` for larger MM9 keys, custom/mod QBit ranges must be
  declared above the reserved core ranges instead of assuming `10000+` is always free.
